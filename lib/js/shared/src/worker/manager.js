...
const createWorkerManager = ({jobName, jobPath, logConfig, maxConcurrency, minIdleCount, maxIdleMilliseconds}) => {
    ...
    const workerPool = LimitedPool({
        ...
        onMsg: ({name, instanceId, action}) => log.debug(`Worker [${name}.${instanceId.substr(-4)}] ${action}`)
    })
    ...
    request$.pipe(
        tap(({jobId, args}) => {
            ...
            _.isEmpty(workerArgs)
                ? log.debug(msg(jobId, 'enqueued with no args'))
                : log.debug(msg(jobId, 'enqueued with args'), workerArgs)
        }),
        mergeMap(({jobId, args, args$}) =>
            workerPool.getInstance$().pipe(
                tap(() => log.debug(msg(jobId, 'dequeued'))),
                mergeMap(worker =>
                    worker.submit$(jobId, args, args$).pipe(
                        ...
                        tap(() => log.debug(msg(jobId, 'complete')))
                    )
                ),
