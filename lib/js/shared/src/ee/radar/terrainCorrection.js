...
const terrainCorrection = function (image, options) {
    // set defaults if undefined
    options = options || {}
    const model = options.model || 'volume'
    const elevation = options.elevation || ee.Image('USGS/SRTMGL1_003').resample()
    ...
    function _correct(image) {
        ...
        const bandNames = image.bandNames().remove('angle')
