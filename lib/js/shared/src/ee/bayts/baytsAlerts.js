...
const baytsAlerts = (recipe, {...args} = {}) => {
    ...
    if (visualizationType && visualizationType !== 'alerts') {
        const delegate$ = aoi$.pipe(
            switchMap(aoi => imageFactory(radarRecipe(aoi, recipe, args), args)),
            shareReplay()
        )
        return {
            getImage$() {
        ...
        return {
            getImage$() {
                return toAlerts$(recipe, reference)
                
            },
            getBands$() {
                return toAlerts$(recipe, reference).pipe(
...
const toAlerts$ = (recipe, reference) => {
    return imageFactory(reference).getImage$().pipe(
        map(historicalStats => {
            return bayts({
                historicalStats,
                ...toDates(recipe),
...
const radarRecipe = (aoi, recipe, {visualizationType}) => {
    const {monitoringEnd, monitoringStart} = toDates(recipe)
    return {
        type: 'RADAR_MOSAIC',
        model: {
            aoi,
