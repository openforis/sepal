...
ansible-playbook ${CONTEXT_DIR}/configure-instances.yml \
    -i ${INVENTORY_FILE} \
    --private-key=~/.ssh/sepal/${REGION}.pem \
    --extra-vars "local_php=false local_sepal=false deploy_environment=$ENV version="$VERSION" skip_cache=$SKIP_CACHE use_custom_host=false secret_vars_file=~/.sepal/secret.yml"
