const {catchError, concat, defer, forkJoin, map, merge, of, switchMap, throwError, toArray} = require('rxjs')
...
module.exports = ee => {
    ...
    const replaceAssetProperties$ = (eeId, properties, maxRetries = 0) =>
        forkJoin({
            properties: getInfo$(ee.Dictionary(properties), 'get properties', maxRetries),
            asset: getAsset$(eeId, maxRetries)
        }).pipe(
            switchMap(({properties, asset}) => {
                const validProperties = _.pickBy(properties, value => _.isNumber(value) || _.isString(value))
                const currentKeys = Object.keys(asset.properties)
                const newKeys = Object.keys(validProperties)
                currentKeys.forEach(key => newKeys.includes(key) || (validProperties[key] = null))
                return $({
                    operation: `set asset properties(${eeId})`,
                    ee: (resolve, reject) => ee.data.setAssetProperties(eeId, validProperties, (result, error) => {
                        return error
                            ? reject(error)
                            : resolve(result)
                    }
        ...
        const list$ = pageToken => $({
            ...
            ee: (resolve, reject) => ee.data.listAssets(parentEEId, {...params, pageToken}, (result, error) => {
                ...
                const assets = serializedAssets
                    ? serializedAssets.map(({Serializable$values: {type, id, name}}) => ({type, id, name}))
                    : []
                return error
                    ? reject(error)
                    : resolve({nextPageToken, assets})
            }),
            maxRetries
        }).pipe(
    ...
    return {setMaxRetries, setUsername, $, getAssetRoots$, listAssets$, getAsset$, replaceAssetProperties$, deleteAsset$, deleteAssetRecursive$, createAssetFolders$, createImageCollection$, getInfo$, getMap$, isNull, sepal}
}
