...
// telegraphic hint (enum/const/type/range/description). A curated knowledge
// list is rendered into a field-guidance section, then a warnings section for
// the facts that flag high-cost / reliability hazards, and rule-derived
// constraints are appended as a separate section. Output is deterministic and
...
function factEntry(fact) {
    ...
    const parts = [`${paths.join(', ')} [${fact.topics.join(', ')}]: ${fact.guidance.join(' ')}`]
    if (fact.tradeoffs) {
