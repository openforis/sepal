const {EMPTY, catchError, concatMap, defaultIfEmpty, defer, finalize, ignoreElements, last, scan, tap, timeout} = require('rxjs')
...
function createTitleGenerator({llm, conversationsStore, tracer, bus}) {
    ...
    function generate$(channel, conversationId, userText, assistantText) {
        return defer(() => {
            if (generating.has(conversationId)) return EMPTY
            ...
            bus.publish({
                type: 'title.prompt',
                level: 'trace',
                message: () => `Title prompt for ${conversationId}: ${truncateDebug(JSON.stringify(titleMessages))}`
            })
            return tracer.span$('title.generate', {conversationId},
                tracer.span$('title.llmRespondTo', {conversationId},
                    llm.respondTo$({
                    ...
                    scan((acc, event) => event.textDelta ? acc + event.textDelta : acc, ''),
                    defaultIfEmpty(''),
                    tap(raw => {
                        const partial = cleanTitle(raw)
                        if (partial) {
                            finalTitle = partial
                            source = 'llm'
                            channel.conversationUpdated({id: conversationId, title: partial})
                        }
                    }),
                    last(),
                    tap(raw => bus.publish({
                    ...
                    concatMap(() => {
                        if (!finalTitle) {
                            finalTitle = fallbackTitle({userText, assistantText})
                            source = finalTitle ? 'fallback' : null
                            if (finalTitle) channel.conversationUpdated({id: conversationId, title: finalTitle})
                        }
                        return finalTitle
                            ? conversationsStore.updateTitle$(conversationId, finalTitle)
                            : EMPTY
                    }),
                    catchError(error => {
                        bus.publish({
                    ...
                    finalize(() => {
                        ...
                        bus.publish({
                            type: finalTitle ? 'title.generated' : 'title.empty',
                            level: 'info',
                            message: finalTitle
                                ? `Title generated for ${conversationId} (${source}): ${JSON.stringify(finalTitle)}`
