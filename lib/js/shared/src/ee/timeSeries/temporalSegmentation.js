...
function DateRange(segments, fromDate, toDate) {
    ...
    function mean(harmonics = 3) {
        ...
        const tBreak = filteredSegments.first().toImage('tBreak')
        const actualTBreak = tBreak
            .where(tBreak.gt(dateConversion.toT(ee.Date(toDate), dateFormat)), 0)
        const magnitude = filteredSegments.first().toImage('.*_magnitude')
            .where(actualTBreak.not(), 0)
        const changeProbability = filteredSegments.first().toImage('changeProb')
            .where(actualTBreak.not(), 0)
        const coefs = filteredSegments.toImage('.*_coefs')
        const intercept = coefs
            .arraySlice(1, 0, 1)
            .arrayProject([0])
