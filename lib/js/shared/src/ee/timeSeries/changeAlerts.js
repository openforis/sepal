...
const changeAlerts = (recipe, {selection: selectedBands, baseBands} = {selection: [], baseBands: []}) => {
                    ...
                    return forkJoin({
                        ...
                        switchMap(({segmentsImage, geometry}) => {
                            return getObservations$(geometry).pipe(
                                map(collection => analyzeChanges({
                                    segmentsImage,
                                    collection,
                                    monitoringStart,
                                    band,
                                    minConfidence: 5,  // TODO Pick the rest from ops
...
function analyzeChanges(args) {
    ...
    var monitoringObservations = changes1d
        .select(0)
        .arrayMask(changes1d.select('date').gte(toFractionalYear(monitoringStart)))
      
    var monitoringObservationCount = monitoringObservations
        .updateMask(monitoringObservations.select(0).arrayLength(0))
        .arrayReduce(ee.Reducer.count(), [0])
        .arrayGet([0])
        .unmask(0)
        .rename('monitoring_observation_count')
      
    var calibrationObservations = changes1d
        .select(0)
        .arrayMask(changes1d.select('date').lt(toFractionalYear(monitoringStart)))
      
    var calibrationObservationCount = calibrationObservations
        .updateMask(calibrationObservations.select(0).arrayLength(0))
        .arrayReduce(ee.Reducer.count(), [0])
        .arrayGet([0])
        .unmask(0)
        .rename('calibration_observation_count')
    ...
    var lastStableDate = lastStable1d
        .select('date')
        .arrayMask(lastStable1d.select('change').not()) // Mask out any eventual changes
        .arrayGet([-1]) // Last date
        .rename('last_stable_date')
    ...
    var firstDetectionDate = firstChange1d
        .select('date')
        .arrayMask(firstChange1d.select('change')) // Mask out any eventual stable observations
        .arrayGet([0]) // First date
        .rename('first_detection_date')
  
    var confirmationDate = firstChange1d
        .select('date')
        .arrayMask(firstChange1d.select('change')) // Mask out any eventual stable observations
        .arrayGet([-1]) // Last date
        .rename('confirmation_date')
