...
// A transform is in its CRS's units, which for EPSG:4326 are DEGREES. isAxisAlignedTransform only inspects signs
// and squareness, so it cannot tell degrees from metres. Downstream consumers - the Random arrangement cell size,
// the minimum-distance floor - read the pixel size as METRES, so a derived grid carries the metre value
// explicitly and it wins over the transform.
describe('gridPixelSize prefers an explicit metre value', () => {
    it('uses pixelSizeMetres when present, over a degree transform', () => {
        expect(gridPixelSize({
            crs: 'EPSG:4326',
    ...
    it('uses pixelSizeMetres over scale', () => {
        expect(gridPixelSize({scale: 999, pixelSizeMetres: 30})).toBe(30)
    })

    it('falls back to the transform when no metre value is present', () => {
        expect(gridPixelSize({crsTransform: [20, 0, 5, 0, -20, 7]})).toBe(20)
    })

    it('falls back to scale when neither a metre value nor a transform is present', () => {
        expect(gridPixelSize({scale: 30})).toBe(30)
    })

    it('ignores a non-positive or non-finite metre value', () => {
        expect(gridPixelSize({scale: 30, pixelSizeMetres: 0})).toBe(30)
        expect(gridPixelSize({scale: 30, pixelSizeMetres: NaN})).toBe(30)
    })
})
