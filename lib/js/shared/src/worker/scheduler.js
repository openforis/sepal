...
const createJobScheduler = ({jobName, jobPath, logConfig, maxConcurrency, minIdleCount, maxIdleMilliseconds}) => {
    ...
    request$.pipe(
        ...
        mergeMap(({jobId, initArgs, args, args$, cmd$}) =>
            workerPool.getInstance$().pipe(
                tap(() => log.debug(jobMsg(jobId, 'dequeued'))),
                mergeMap(worker =>
                    worker.submit$({jobId, initArgs, args, args$, cmd$}).pipe(
    ...
    return {
        submit$({initArgs, args, args$, cmd$}) {
            const jobId = uuid()
            request$.next({jobId, initArgs, args, args$, cmd$})
            return jobResponse$(
                response$.pipe(
                    filter(({jobId: currentJobId}) => currentJobId === jobId),
                    map(({result}) => result),
