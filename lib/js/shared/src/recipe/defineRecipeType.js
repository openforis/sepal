...
//
// The optional image output is the type's IMAGE_OUTPUT provider. Optional, because most types have not been
// migrated and not declaring one must stay distinguishable from declaring one - so an undeclared type keeps no
// key at all rather than an empty one. Its structure is checked by the provider module that owns those rules,
...
import {validateImageOutputProvider} from './output/provider.js'
...
export const defineRecipeType = definition => {
    const {
        type, directSources, imageOutput, segmentSource, historicalStatsSource, opticalCollectionDefaults
