...
const {getLogger, getLogConfig} = require('#sepal/log')
const {initWorker$} = require('./factory')
const {StaticPool} = require('./staticPool')

const log = getLogger('scheduler')

const WORKER_INSTANCES = 40
const WORKER_CREATION_DELAY_MS = 0
const WORKER_CREATION_CONCURRENCY = 1

let scheduler = null

const initScheduler = () => {
    scheduler = createScheduler(getLogConfig())
}
...
const createScheduler = logConfig => {
    ...
    const workerPool$ = StaticPool({
        ...
        createConcurrency: WORKER_CREATION_CONCURRENCY,
        createDelayMs: WORKER_CREATION_DELAY_MS,
        create$: workerId => initWorker$({workerId, logConfig}),
    })
