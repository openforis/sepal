...
const setupWorker = ({workerId, jobName, jobPath, worker, port}) => {
    ...
    const submit$ = ({requestTag, initArgs, args, args$, cmd$}) => {
        const {in$: request$, out$: response$} = transport.createChannel(id, 'job')

        const start = () =>
            request$.next({start: {requestTag, jobPath, initArgs, args}})

        const stop = () =>
            request$.complete()

        args$ && args$.subscribe(
            value => request$.next({next: {requestTag, value}})
            // [TODO] handle error
        )

        cmd$ && cmd$.subscribe(
            cmd => request$.next({next: {requestTag, cmd}})
        )

        start()

        return response$.pipe(
            catchError(error => of({requestTag, error: true, value: error})),
            filter(message => message.requestTag === requestTag),
            finalize(() => stop())
        )
    }
