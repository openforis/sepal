...
const simplifyString = (s, {trim = true, removeAccents = true, removePunctuation = false, removeNonAlphanumeric = true, removeRepeatedSpaces = true} = {}) => {
    if (!_.isString(s)) {
        return s
    }
...
const splitString = s => s ? s.split(/\s/) : []

const toSafeString = s =>
    s
        ? simplifyString(s, {
...
module.exports = {simplifyString, splitString, toSafeString, escapeRegExp}
