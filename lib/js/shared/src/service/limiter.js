const {Subject, ReplaySubject, from, zip, concat, of} = require('rxjs')
...
const withLimiter$ = limiter =>
    observable$ => {
        ...
        //     maxConcurrency: 1
        // })()
        const token$ = from(service.submit$(limiter, requestId))

        const releaseToken = token => {
            releaseToken$.next()
            log.fatal(`Returning token ${token}`)
        }
        //
        // return token$.pipe(
