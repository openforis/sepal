    ...
    ({
     ...
     {selection: selectedBands} = {selection: []}
        ...
        const calculateClassProbability = (trainingData, imageToClassify, classification) => {
            const probabilities = legend.entries.map(
                ({value}) =>
                    calculateProbability(trainingData, imageToClassify, `probability_${value}`)
                        .where(classification.neq(value), 0)
            )
            return ee.Image(probabilities).reduce(ee.Reducer.max())
                .rename('class_probability')
        }

        const calculateProbability = (trainingData, imageToClassify, bandName) => {
            const legendValue = parseInt(bandName.match(/probability_(.*)/)[1])
            const classifier = createClassifier(classifierConfig)
                .train(
                    trainingData
                        .map(feature => feature.set('class', feature.getNumber('class').eq(legendValue))),
                    'class'
                )
        ...
        return {
                    ...
                    switchMap(images => {
                        ...
                        return getTrainingData$(imageToClassify.bandNames()).pipe(
                            map(trainingData => {
                                ...
                                if (selectedBands.includes('regression') || selectedBands.includes('class_probability')) {
                                ...
                                const probabilityBands = selectedBands
                                    ...
                                    .map(bandName => calculateProbability(trainingData, imageToClassify, bandName))
                                bands.push(probabilityBands)
            ...
            getVisParams$() {
                ...
                if (selectedBands.includes('class')) {
                    return of({bands: 'class', min, max, palette})
                } else if (selectedBands.includes('regression')) {
                    return of({bands: 'regression', min, max, palette})
                } else if (selectedBands.includes('class_probability')) {
                    return of({bands: 'class_probability', min: 0, max: 100, palette: 'red, orange, white, green, blue'})
                } else {
                    const probabilityBands = selectedBands
                        .filter(bandName => bandName.startsWith('probability_'))
                    if (probabilityBands.length) {
                        return of({bands: probabilityBands[0], min: 0, max: 100, palette: 'red, orange, white, green, blue'})
                    } else {
                        throw new Error('Expected selected bands to contain class, regression, class_probability, or probability_*')
                    }
                }
            },
            getGeometry$() {
