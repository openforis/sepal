...
// Grid pixel size in METRES. Scale is the whole grid definition: an affine transform is expressed in its CRS's
// own units - degrees for EPSG:4326 - so it could never be read as a metre spacing, and alignment to a source's
// pixel grid is decided by Earth Engine rather than configured here.
export const gridPixelSize = ({scale} = {}) => Number(scale)
...
// floor). An indeterminate grid (no valid scale) is also valid here: the grid-definition error
