...
SET @do_copy := (SELECT IF(
    ...
    'INSERT INTO budget.`user_budget` SELECT * FROM sdms.`user_budget`',
    'DO 0'));
...
SET @do_copy := (SELECT IF(
    ...
    'INSERT INTO budget.`user_monthly_storage` SELECT * FROM sdms.`user_monthly_storage`',
    'DO 0'));
PREPARE _s FROM @do_copy; EXECUTE _s; DEALLOCATE PREPARE _s;
...
SET @do_copy := (SELECT IF(
    ...
    'INSERT INTO budget.`user_spending` SELECT * FROM sdms.`user_spending`',
    'DO 0'));
PREPARE _s FROM @do_copy; EXECUTE _s; DEALLOCATE PREPARE _s;
...
SET @do_copy := (SELECT IF(
    ...
    'INSERT INTO budget.`budget_update_request` SELECT * FROM sdms.`budget_update_request`',
    'DO 0'));
PREPARE _s FROM @do_copy; EXECUTE _s; DEALLOCATE PREPARE _s;
...
SET @do_seed := (SELECT IF(
    ...
    'INSERT INTO budget.`open_session_use` (session_id, username, instance_type, from_time, to_time) SELECT id, username, instance_type, creation_time, CASE WHEN state=''CLOSED'' THEN update_time ELSE NULL END FROM sdms.`worker_session` WHERE state IN (''PENDING'',''ACTIVE'') OR (state=''CLOSED'' AND update_time >= DATE_FORMAT(NOW(),''%Y-%m-01''))',
    'DO 0'));
