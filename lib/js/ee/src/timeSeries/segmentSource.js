...
import {INHERITED, inheritedSchemaSource, NOT_INHERITED} from '#sepal/recipe/output/inheritedSchemaSource'
...
// What a recipe says about the segments it produces, or - when it says nothing but declares that it
// preserves an input - what that input says. Only a declared preserving role is followed, so a mask, a
// fill or an AOI can never become the producer.
const producerFacts$ = (record, savedDateFormat) => {
    ...
    if (declared) {
        return declared.segmentsAsset
            ? assetFacts$(declared.segmentsAsset(record.model), declared.selectableBaseBands !== false, savedDateFormat)
            : of({
