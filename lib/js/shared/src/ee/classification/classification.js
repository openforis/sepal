...
const {concat, defer, of, zip} = require('rx')
...
const classify =
    ...
    {selection: selectedBands} = {selection: []}
        ...
        const normalizeImage = image => {
            ...
            const normalized = ee.Image(ee.Image(
        ...
        const imageToClassify$ = defer(() => zip(
            ...
            map(imageToClassify => classifierConfig.type === 'SVM' ? normalizeImage(imageToClassify) : imageToClassify)
        )
        ...
        return {
            ...
            getGeometry$() {
                return imageToClassify$().pipe(
                    map(imageToClassify => imageToClassify.geometry(scale))
                )
            },
