...
import {MAX_DENSITY_OFFSETS, minLatticeExponent, targetLatticeDiameter} from './systematicLatticeMath.js'
...
// area/sampleSize) means the stratum can't be densified (0). Uses the shared lattice math so the raster and
// this decision can't drift apart.
export const systematicStratumMaxOffset = (stratum, sampleArrangement) => {
    const offset = Math.floor(Math.log2(targetLatticeDiameter(stratum))) - minLatticeExponent(sampleArrangement)
...
// Finalize systematic samples. rowMetadata (default, preview/feature-source and SEPAL/CSV) attaches full
// per-row sample + reproduction metadata; rowMetadata: false (asset export) keeps rows minimal
// (id/stratum/selectedLevel) and moves reproduction/allocation metadata to the collection level.
