...
const {initWorkerMetrics, getMetrics} = require('#sepal/metrics')
...
const initWorker = ({workerId, jobName, port}) => {
    ...
    const setupMetrics = ({metricsRequest$, metricsResponse$}) => {
        metricsRequest$.subscribe({
            next: async () => metricsResponse$.next(await getMetrics())
        })
    }
    ...
    const transport = PortTransport({
        ...
        onChannel: {
            ...
            'metrics': ({in$: metricsResponse$, out$: metricsRequest$}) => setupMetrics({metricsRequest$, metricsResponse$})
        }
    })
