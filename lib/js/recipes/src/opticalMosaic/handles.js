...
const HANDLES = [
    {
        ...
        userRequired: true,
        valueGuidance: 'Whole-object replacement. Set ONLY from a real AOI object supplied by the prepared packet, the user instruction, an AOI lookup result, or a clarification answer. Named countries/admin areas must be resolved through the AOI lookup tools and the returned object must be used verbatim. Do NOT hand-build country AOIs, geocode place names, or invent polygon coordinates. If no real AOI object or clear AOI lookup match is available, ask ONE clarification question.',
        ...
        allowedItems: [
            ...
            {value: 'EE_TABLE', label: 'Earth Engine feature table', shape: {type: 'EE_TABLE', id: 'GEE asset id (country table: users/wiell/SepalResources/gaul)', keyColumn: 'string', key: 'string or number', level: 'optional'}}
        ],
        examples: [
            {type: 'POLYGON', path: [[36.7, -1.4], [37.0, -1.4], [37.0, -1.1], [36.7, -1.4]]},
            {type: 'EE_TABLE', id: 'users/wiell/SepalResources/gaul', keyColumn: 'id', key: 21, level: 'COUNTRY', buffer: 0}
        ]
    },
    {
        name: 'datasets',
        ...
        selectionGuidance: 'Pick for named source/sensor requests and historical date windows. Match era: LANDSAT_TM for 1980s/1990s, LANDSAT_7 from 1999, LANDSAT_8 from 2013, LANDSAT_9 from 2021, SENTINEL_2 from 2015.',
        valueGuidance: 'Whole object replacement. Keys are source groups; values are arrays of dataset codes. Match dataset to analysis window: LANDSAT_TM 1982+ (Landsat 4-5, only choice for 1980s/1990s), LANDSAT_7 1999+, LANDSAT_8 2013+, LANDSAT_9 2021+, SENTINEL_2 2015+. Selected datasets must have coverage within [seasonStart, seasonEnd) — do not pair LANDSAT_8/LANDSAT_9 with pre-2013 windows. Removing source groups/datasets is a high-tradeoff speed lever: fewer scenes, but less temporal/source coverage. Do not use for generic first-pass speed unless the user asks to reduce sources/data volume or accepts speed over coverage.',
        summaryGuidance: 'Talk about which satellites the mosaic uses (e.g. "now uses Landsat only").',
        performanceNote: 'More source groups + datasets widen the candidate scene pool; memory/latency scale with that. High-tradeoff: ask before reducing sources unless explicitly requested.',
        allowedKeys: {
    ...
    },
    {
        ...
        description: 'Scene-level cloud filter applied before per-pixel masking; integer percentage 0-100. Lower values admit fewer candidate scenes.',
        valueGuidance: 'Integer 0-100; 75 is the GUI default. This is mainly a performance/candidate-pool lever, not a direct pixel-cloud mask. Lower values can avoid some mask misses but may create gaps.',
        summaryGuidance: 'Explain this as candidate-scene filtering, not cloud-mask strength.',
        performanceNote: 'Lower limits reduce candidate scenes and usually reduce memory/latency, but can reduce coverage.',
        range: {min: 0, max: 100},
        examples: [50, 75, 90]
    },
    {
        ...
        valueGuidance: 'Whole-array replacement. Combine SR (surface reflectance), BRDF (view-angle correction), CALIBRATE (cross-sensor calibration). SR and CALIBRATE are mutually exclusive; SR or CALIBRATE is required when both Landsat and Sentinel-2 source groups are selected. For speed, prefer removing BRDF while preserving the current SR-vs-CALIBRATE mode unless the user asks to change it.',
    ...
    },
    {
        ...
        valueGuidance: 'Use SELECT only when the user explicitly asks for manual scene selection / picked scenes. Do not use SELECT as a generic speed optimization. ALL is required when both Landsat and Sentinel-2 source groups are selected; with mixed sources, reduce sceneCloudLimit or source groups instead.',
...

    {
        ...
        valueGuidance: 'Whole-array replacement. Empty disables cloud masking. For residual-cloud requests, preserve active/applicable methods and tighten their companions; prefer Sentinel-2 Cloud Score+ over legacy Sentinel-2 Cloud Probability unless the user explicitly asks otherwise.',
        ...
        allowedItems: [
            ...
            },
            {
                ...
                recommendation: 'Preferred Sentinel-2 cloud method for generic cloud-improvement requests.',
            ...
            },
            {
                ...
                appliesTo: ['SENTINEL_2'],
                alternativeGroup: 'sentinel2CloudMask',
                recommendation: 'Legacy Sentinel-2 option; prefer Cloud Score+ for generic cloud-improvement requests.',
    ...
    },
    {
        ...
        valueGuidance: 'Only applies when legacy Sentinel-2 Cloud Probability is active. Prefer Cloud Score+ for new/generic Sentinel-2 cloud masking.',
        summaryGuidance: 'Lower = stricter cloud filtering.',
        range: {min: 0, max: 100},
        examples: [40, 60]
    },
...

    {
        ...
        performanceNote: 'Not a render-speed lever by itself; for speed, remove BRDF via radiometric corrections.',
    ...
    },
    {
        ...
        selectionGuidance: 'Pick with seasonStart and seasonEnd for bare-year requests or explicit date-window requests.',
        valueGuidance: 'Must be on or after 1982-08-22 (Landsat 4 launch). Bare-year request `YYYY` → targetDate `YYYY-07-02` (mid-year). Changing it usually requires moving seasonStart and seasonEnd into the new window.',
        format: 'YYYY-MM-DD',
        examples: ['2024-07-02']
    },
    {
        ...
        label: 'Season start',
        description: 'Inclusive start of the analysis period (YYYY-MM-DD). With seasonEnd, defines the half-open window [seasonStart, seasonEnd). For calendar year 2024: seasonStart=2024-01-01, seasonEnd=2025-01-01.',
        selectionGuidance: 'Pick with targetDate and seasonEnd for bare-year requests or explicit date-window requests.',
        valueGuidance: 'Must be in [targetDate - 1 year + 1 day, targetDate]. Bare-year request `YYYY` → seasonStart `YYYY-01-01` (calendar start). Moving it later shortens the candidate season before targetDate; faster but may reduce clear observations/coverage. High-tradeoff: ask before narrowing season unless the user requested a shorter date range or accepted speed over coverage.',
        performanceNote: 'Later seasonStart shortens the candidate scene window and can reduce memory/latency. High-tradeoff: not a generic first-pass speed lever.',
        format: 'YYYY-MM-DD',
        examples: ['2024-01-01']
    },
    {
        ...
        label: 'Season end',
        description: 'Exclusive end of the analysis period (YYYY-MM-DD). With seasonStart, defines the half-open window [seasonStart, seasonEnd). For calendar year 2024: seasonStart=2024-01-01, seasonEnd=2025-01-01.',
        selectionGuidance: 'Pick with targetDate and seasonStart for bare-year requests or explicit date-window requests.',
        valueGuidance: 'Must be in [targetDate + 1 day, targetDate + 1 year]. Bare-year request `YYYY` → seasonEnd `(YYYY+1)-01-01` (half-open calendar end). Do not default to targetDate + 1 year for a bare-year request; use the calendar-year end. Moving it earlier shortens the candidate season after targetDate; faster but may reduce clear observations/coverage. High-tradeoff: ask before narrowing season unless the user requested a shorter date range or accepted speed over coverage.',
        performanceNote: 'Earlier seasonEnd shortens the candidate scene window and can reduce memory/latency. High-tradeoff: not a generic first-pass speed lever.',
        format: 'YYYY-MM-DD',
        examples: ['2025-01-01']
    },
    {
