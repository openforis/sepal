...
// while ordered bands own their physical schema and optional per-band export requirements. Evidence entries
// are opaque and retained by identity. Validation accumulates diagnostics and never returns a partial
// description. This module performs no resolution, policy inference or Earth Engine task conversion.
...
export const imageOutputDescription = input => {
    ...
    return {
        description: {
            ...
            output: {
                kind: 'IMAGE',
                bands: bands.map(({name, dataType, pyramidingPolicy}) => ({
                    name,
                    ...(dataType !== undefined && {
