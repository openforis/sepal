...
function canonicalizeCorrections(composite, {hasMultipleSourceGroups}) {
    ...
    const corrections = composite.corrections.filter(correction =>
        correction !== (hasMultipleSourceGroups ? 'SR' : 'CALIBRATE')
    )
    if (hasMultipleSourceGroups && !corrections.includes('CALIBRATE')) {
        corrections.push('CALIBRATE')
    }
    composite.corrections = corrections
}
