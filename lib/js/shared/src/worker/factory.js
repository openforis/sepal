...
const {catchError, filter, finalize, first, map} = require('rx/operators')
...
const setupWorker = ({workerId, jobName, jobPath, worker, port}) => {
    ...
    transport.onChannel(
        ({channelId, in$: response$, out$: request$}) => {
            const prefix = `${id}.service-`
            if (channelId.startsWith(prefix)) {
                const serviceName = channelId.substr(prefix.length)
    ...
    const submit$ = ({jobId, initArgs, args, args$}) => {
