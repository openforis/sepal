...
import applyBRDFCorrection from '#sepal/ee/optical/imageProcess/applyBRDFCorrection'
import dataSetSpecs from '#sepal/recipe/optical/dataSetSpecs'
import {HISTOGRAM_MATCHED_BAND_NAMES} from '#sepal/recipe/planet/planetBands'
    ...
    function landsatReference(referenceStartDate, referenceEndDate, yearOffset, aoi) {
        ...
        return ee.ImageCollection(dataSetSpec.collectionName)
            ...
            .map(image => {
                ...
                ).select(HISTOGRAM_MATCHED_BAND_NAMES)
            })
            .median()
    }
...
const histogramMatch = (target, reference, referenceScale, targetScale) => {
    ...
    const bandNames = HISTOGRAM_MATCHED_BAND_NAMES
...
export {createReference, histogramMatch}
