...
describe('auditDirectSources', () => {
    ...
    it.each([
        ...
        ['MASKING', maskingRecipe()],
        ['MOSAIC', opticalMosaicRecipe()],
        ['RADAR_MOSAIC', radarMosaicRecipe()],
        ['PLANET_MOSAIC', planetMosaicRecipe()],
        ['TIME_SERIES', timeSeriesRecipe()],
        ['PHENOLOGY', phenologyRecipe()],
        ['CHANGE_ALERTS', changeAlertsRecipe()],
        ['CHANGE_ALERTS over an asset', changeAlertsRecipe({reference: {type: 'ASSET', id: 'projects/p/assets/segments'}})],
        ['PYEO_ALERTS', pyeoAlertsRecipe()],
        ['BAYTS_HISTORICAL', baytsHistoricalRecipe()],
        ['BAYTS_ALERTS', baytsAlertsRecipe()],
        ['STACK', stackRecipe()],
        ...
        ['CLASS_CHANGE', classChangeRecipe()],
        ['INDEX_CHANGE', indexChangeRecipe()],
        ['SAMPLING_DESIGN', samplingDesignRecipe()],
        ['SAMPLING_DESIGN with a skipped stratification', samplingDesignRecipe({stratification: {skip: true}})]
    ...
    it('does not claim completeness for an undefined recipe type', () => {
        expect(codes({id: 'retired-1', type: 'RETIRED_RECIPE', model: {aoi: recipeAoi()}}))
            .toEqual(['UNSUPPORTED_RECIPE_TYPE'])
    })
})

describe('auditDirectSources over MASKING', () => {
    // The role this packet deliberately does not declare. Fill is the next input the recipe grows, and the
    // guard exists so that adding it to the model without declaring it fails here rather than disappearing
    // from the dependency graph.
    it('rejects a fill image until the fill role is declared', () => {
