...
const rules = [
    ...
    },
    {
        ...
        description: 'When Landsat and Sentinel-2 source groups are both selected, corrections must include cross-sensor calibration.',
        ...
        validate: model => {
            ...
            const corrections = model?.compositeOptions?.corrections || []
            if (corrections.includes('CALIBRATE')) return []
            return [{path: '/compositeOptions/corrections', message: 'cross-sensor calibration is required when both Landsat and Sentinel-2 source groups are selected'}]
        }
    },
    {
