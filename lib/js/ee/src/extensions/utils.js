const {concatWith, mergeMap, catchError, concat, concatMap, defer, ignoreElements, toArray, map, of, switchMap, throwError, EMPTY} = require('rxjs')
const http = require('#sepal/httpClient')
const {promise$, autoRetry, swallow} = require('#sepal/rxjs')
...
const {applyDefaults} = require('#sepal/util')
...
module.exports = ee => {
    ...
    const userTag = () => tag('User', config.username || 'ANON')

    const $ = ({description, operationId = uuid(), operation, maxRetries = config.maxRetries, skipAuthTokenCheck}) =>
        defer(() =>
            of(true).pipe(
                switchMap(() =>
                    eeLimiter$(
                        promise$(
                            (resolve, reject) => {
                                ...
                                try {
                                    if (!skipAuthTokenCheck && sepal.getAuthType() === 'USER' && !ee.data.getAuthToken()) { // When we authenticate, this gives an error
                                        log.warn(userTag(), `${description}: Missing authToken`)
        ...
        const list$ = pageToken => $({
            ...
            operation: (resolve, reject) => ee.data.listAssets(parentEEId, {...params, pageToken}, (result, error) => {
                if (error) {
                    ...
                    const assets = serializedAssets
                        ? serializedAssets.map(({Serializable$values: {type, id, name, updateTime}}) => ({type: TYPE_MAP[type], id, name, updateTime}))
                        : []
            ...
            catchError(error => {
                if (error.cause.startsWith('Google Earth Engine API has not been enabled in project:')) {
                    log.warn(`${userTag()} Cannot list assets for ${parentEEId}.`)
                } else {
                    log.warn(`${userTag()} Error while trying to list assets for ${parentEEId}.`, error)
                }
                return of({assets: []})
            }),
