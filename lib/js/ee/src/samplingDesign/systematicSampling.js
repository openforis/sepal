...
import {BASE_GRID_SLACK, ROOT_DIAMETER_EXPONENT} from './systematicLatticeMath.js'
...
// Nested systematic lattice image; higher selected levels skip rows rather than changing point geometry.
export function stratifiedSystematicSampleImage(args) {
    ...
    // Base lattice before per-stratum level selection.
    function createHexSamplesImage(args) {
        var diameter = args.diameter
        var scale = args.scale
        var proj = args.proj || ee.Projection(crs)
        ...
        // The centroid mask below uses the true i/j (cell centers), while the level/coset uses the
        // seed-shifted indices - translating the selected nested coset by whole cells (zero for FIXED).
...
// ---- exact-first, geoseeded, lookup-enriched stratified candidate generation ----
// stratifiedSystematicExactCandidates and stratifiedSystematicFinalSamples are self-contained (only the global
// `ee` identifier plus nested helpers) so their declarations are copy/pastable into the EE Code Editor unchanged;
// they are exported from the module below. Keep the bodies textually identical to the retained Code Editor
// harness scripts under tmp/sudan-systematic/geoseeded/.

// Exact-first, geoseeded, lookup-enriched stratified candidate generator. Returns a candidate FeatureCollection
// whose geometries are the exact lattice points, filtered to the AOI, carrying only stratum, i, j and level.
//
// One label-only reduceToVectors, then scalar enrichment in the same pass. The label is COMPACT (one 512-block
// per stratum + the i/j residues, so it stays below allocation.length * 512) and thus survives reduceToVectors
// without float64 rounding; full i/j are reconstructed from the vectorized centroid. The geoseeded origin, lattice
// spacing, marker coordinates and exact geometry all live in ONE projection: pixelCoordinates(crs[,transform])
// with distances via nominalScale() - metres for a scale grid, pixel-index units for a transform grid. The origin
// is a seed-only global anchor (uniform over one period-aligned lattice period; independent of
// AOI/allocation/stratum/densityOffset); FIXED => origin (0,0).
//
// This function is projection-agnostic: it assumes projected, metre-based coordinates. The supported-CRS contract
// (the curated EASE-Grid 2.0 catalog) is enforced at the task boundary before the graph is built, so there is no CRS check
// or EE projection introspection here. Scale and crsTransform are mutually exclusive; a transform must be
// north-up, axis-aligned, square and non-zero.
function stratifiedSystematicExactCandidates(args) {
    ...
    // ~2^24 origin - so the persisted i/j stay small and the centroid reconstruction is well-scaled.
    ...
    // Vectorized centroids come back in the vectorization grid's own pixel basis (geometryInNativeProjection),
    // where one pixel spans `pixelSize` CRS units, while the marker basis above counts in units of nominalScale.
    // One factor converts either grid: for a transform grid nominalScale == pixelSize, so it is 1 (the
    // vectorization grid and the marker projection are the same grid).
    var nativeToGrid = ee.Number(pixelSize).divide(nominalScale)
    ...
    // geometryInNativeProjection returns the temporary centroid in this vectorization grid's pixel basis rather
    // than WGS84, so reconstruction scales it by nativeToGrid instead of reprojecting every feature. It affects
    // only that temporary centroid: the final geometry is still replaced with the exact lattice point, and pixel
    ...
    var vectorized = candidateImage.select('label').reduceToVectors(reduceArgs)

    return reconstruct(vectorized)
        .filterBounds(region)
        .select(['stratum', 'i', 'j', 'level'])

    // Exact-first marker: each accepted pixel is the class pixel containing one stratum's exact lattice point. The
    // label is COMPACT - stratumIndex*512 + (posmod(j,32)*16 + posmod(i,16)) + 1, so it stays below
    // allocation.length * 512 - and reduceToVectors carries it without float64 rounding. A full i/j pack
    // (stratumIndex*2^52 + ...) exceeds 2^53 for stratumIndex >= 2 and silently collides. Equal labels recur only
    // every 16 cells in i / 32 in j; with dx >= 2*pixel and dy > pixel, same-label candidates never share adjacent
    // pixels, so connected components
    // never merge. Full i/j are recovered from the centroid in reconstruct().
    function stratumCandidateLabels(e) {
        ...
        return label.updateMask(accepted)
    }

    // Recover full i/j from each vectorized centroid, then materialize the exact geometry, in a single map over
    // the vectorized result. The compact label splits back into its stratumIndex (one 512-block per stratum,
    // selecting the layout) and its residue (which indexes the nested-level table directly). The centroid is
    // within half a pixel of the exact point; with dy > pixel and dx >= 2*pixel the nearest lattice point is
    // unambiguous, so round() returns exactly the marker's i/j.
    function reconstruct(candidates) {
        return candidates.map(function (feature) {
            ...
            var native = nativeCoordinates(feature)
            var j = native.y.subtract(originY).divide(dy).round()
            var parity = j.mod(2).add(2).mod(2)
            var i = native.x.subtract(originX).subtract(parity.multiply(dx.divide(2))).divide(dx).round()
    ...
    // Centroid in the SAME basis as pixelCoordinates: CRS units for a scale grid, pixel indices for a transform
    // grid. geometryInNativeProjection already delivers it in the vectorization grid's basis, so this is a
    // single scaling - no per-feature reprojection, and no affine, since the vectorization grid carries the
    // transform grid's own origin and sign.
    function nativeCoordinates(feature) {
        var coords = ee.List(feature.geometry().coordinates())
        return {
            x: ee.Number(coords.get(0)).multiply(nativeToGrid),
            y: ee.Number(coords.get(1)).multiply(nativeToGrid)
        }
    }
