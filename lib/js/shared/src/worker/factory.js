...
const {metrics$} = require('#sepal/metrics')
...
const setupWorker = ({workerId, jobName, jobPath, worker, port}) => {
    ...
    const {out$: workerMetrics$} = transport.createChannel('metrics')

    const metricsSubscription = workerMetrics$.subscribe({
        next: metrics => metrics$.next(metrics)
    })
