...
// Reproduction metadata: configured settings plus the density chosen by the systematic draw.
export const REPRODUCTION_PROPERTY_NAMES = [
    ...
    'minDistance',
    'scale',
    // Configured placement grid CRS (there is no user-facing transform).
    'crs',
    'gridCrs',
    // Configured Stratification CRS: the grid the categorical source is interpreted on, and its transform when
    // the grid is transform-defined (absent otherwise).
    'stratificationCrs',
    'crsTransform',
...
// `actualSampleSize` is the per-stratum count of the FINALIZED collection. It is normally read from the
// materialized rows via aggregate_histogram; a generator that draws an exact count (unstratified randomPoints
// draws exactly N) may instead supply `knownCounts` ({stratumString: count}) so the collection is not
// histogrammed to recover a number it produced by construction.
export const addSampleProperties = (collection, allocation, knownCounts) => {
    ...
    // Per-stratum count; keys are stratum values as strings.
    const actualByStratum = knownCounts != null
        ? ee.Dictionary(knownCounts)
...
export const collectionMetadata = ({allocation, reproduction, sampleCountByStratum}) => {
    ...
    const metadata = {}
    // Omit properties the draw does not configure at all (random has no minDistance), while keeping the ones
    // it explicitly nulls. Rows keep the fixed column set instead, blanking what does not apply.
    REPRODUCTION_PROPERTY_NAMES.filter(name => name in full).forEach(name => {
