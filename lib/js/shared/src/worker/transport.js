...
const transport = ({from = 'main', to = 'main', port}) => {

    const msg = msg => [
        transportTag(from, to),
        msg
    ].join(' ')
    ...
    const addChannelListener = ({key, listener, channelId, conversationId, direction}) => {
        if (state.channelListeners[key]) {
            log.warn(() => msg(`${channelTag(channelId, conversationId, direction)} cannot add duplicate ${channelListenerTag(key)}`))
        } else {
            state.channelListeners = {...state.channelListeners, [key]: listener}
            log.debug(() => msg(`${channelTag(channelId, conversationId, direction)} added ${channelListenerTag(key)} (${_.keys(state.channelListeners).length} active)`))
        }
    }

    const removeChannelListener = ({key, channelId, conversationId, direction}) => {
        if (state.channelListeners[key]) {
            state.channelListeners = _.omit(state.channelListeners, key)
            log.debug(msg(`${channelTag(channelId, conversationId, direction)} removed ${channelListenerTag(key)} (${_.keys(state.channelListeners).length} active)`))
        } else {
            log.warn(msg(`${channelTag(channelId, conversationId, direction)} cannot remove non-existing ${channelListenerTag(key)}`))
        }
    }
    ...
    const createChannel = ({channelId, conversationId, direction, in$, out$}) => {
        ...
        const channelPort = {
            ...
            addMessageHandler: messageHandler => {
                ...
                addChannelListener({key, listener, channelId, conversationId, direction})
                return () => removeChannelListener({key, channelId, conversationId, direction})
            }
        }
