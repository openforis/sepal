...
const worker$ = ({recipe, bands, visParams}) => {
    ...
    const getMap$ = (image, {type, bands, min, max, inverted, gamma, palette}) => {
        ...
        switch(type) {
        case 'continuous': return ee.getMap$(image.select(bands), {bands, ...range(), gamma, palette})
        case 'rgb': return ee.getMap$(image.select(bands), {bands, ...range(), gamma, palette})
        case 'hsv': return ee.getMap$(toHsv(image.select(bands)))
