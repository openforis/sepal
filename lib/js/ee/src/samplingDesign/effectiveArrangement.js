...
export const effectiveArrangement = ({stratification, sampleArrangement}) => {
    ...
    if (!isStratificationSkipped(stratification)) {
        // Scale is metres and is the whole grid. Whether the draw lands on the source's own pixel grid is
        // decided inside Earth Engine, by comparing the selected band's projection with this one.
