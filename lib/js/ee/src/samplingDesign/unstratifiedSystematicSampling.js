...
// point geometry to the final (small) selected/thinned set.
//
// TODO(production): when systematicExport.js routes unstratified designs here, bump ALGORITHM_VERSION in
// sampleProperties.js - this emits EXACT analytic lattice points (not raster-snapped centroids) and, for
// EXACT thinning, uses an index-derived idkey instead of the geometry-derived toId. Not wired yet: the raster
// path remains production while helper correctness and live parity are reviewed.

// Shared EE lattice layout for the unstratified design (single stratum): diameter from the CLIENT-side area
// (already injected at the export boundary), then dx/dy/offset in the sampling projection - the SAME
// projection, spacing, root-phase and origin phase as stratifiedSystematicSampleImage, so a small AOI and a
// global AOI land on the same global lattice. Spacing is constrained ONLY by minDistance
// (unstratifiedLatticeDiameter); `scale` deliberately does NOT enter - it must not limit an analytical grid.
...
// Materialize EXACT projected point geometry from stored i, j (deferred until after level selection/thinning,
// so it runs over the small final set). Sign-safe row parity mirrors exactLatticePoint. Filters to the AOI
// and preserves stratum/level/idkey.
