            ...
            getImage$() {
                return zip(
                    ...
                    map(([fromEEImage, toEEImage]) => {
                        return calculateClassChange({
                            fromEEImage,
                            fromValues: fromImage.bands[fromImage.band].values.sort(),
                            fromBand: fromImage.band,
                            toEEImage,
                            toValues: toImage.bands[toImage.band].values.sort(),
                            toBand: toImage.band,
                            minConfidence
                        }).selectExisting(_.uniq(selectedBands))
                    })
                )
            },
            getBands$() {
...
const calculateClassChange = ({
    ...
    const fromValuesWithProbabilities = findValuesWithProbabilities(fromEEImage, fromValues)
    const toValuesWithProbabilities = findValuesWithProbabilities(toEEImage, toValues)
    ...
    const confidence = ee.ImageCollection([toCertainty, fromCertainty])
        .mean()
        .updateMask(fromValue.neq(toValue))
        .rename('confidence')
