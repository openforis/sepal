...
const Transport = ({transportId, messageOut, onChannel}) => {
    ...
    const handleCreateChannel = ({transportId: currentTranportId, conversationGroupId, conversationId}, onChannel) => {
        const byCallback = ({conversationGroupId, conversationId, onChannel}) =>
            onChannel(
                createChannel({conversationGroupId, conversationId, direction: REVERSE})
            )
    
        const byCallbackMap = ({conversationGroupId, conversationId, handler}) =>
            handler(
                createChannel({conversationGroupId, conversationId, direction: REVERSE})
            )

        const byStreams = ({conversationGroupId, conversationId, in$, out$}) =>
        ...
        if (_.isEqual(currentTranportId, transportId) && conversationGroupId && conversationId && onChannel) {
            if (_.isFunction(onChannel)) {
                byCallback({conversationGroupId, conversationId, onChannel})
            } else {
                const handler = onChannel[conversationGroupId]
                if (_.isFunction(handler)) {
                    byCallbackMap({conversationGroupId, conversationId, handler})
                } else if (_.isPlainObject(handler)) {
                    const {in$, out$} = handler
                    byStreams({conversationGroupId, conversationId, in$, out$})
                } else {
                    log.warn(conversationGroupId)
    ...
    return {
        createChannel: (conversationGroupId, conversationId = uuid()) => {
            messageOut({createChannel: {transportId, conversationGroupId, conversationId}})
            return createChannel({transportId, conversationGroupId, conversationId, direction: FORWARD})
        },
        messageIn: message => {
