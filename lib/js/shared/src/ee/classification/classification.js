...
const classify =
    ...
    {selection: selectedBands} = {selection: []}
                ...
                switchMap(images =>
                    concat(
                        ...
                        map(trainingDataSets => {
                            log.fatal(ee.FeatureCollection(trainingDataSets)
                                .flatten()
                                .set('band_order', bands || ee.Image(images).bandNames()).getInfo())
                            return ee.FeatureCollection(trainingDataSets)
                                .flatten()
        ...
        const calculateProbabilities = (trainingData, imageToClassify) => {
            ...
            ).addBands(ee.Image(bandNames.map(bandName => ee.Image(0).rename(bandName)))).select(bandNames)
        }
