const {Subject, ReplaySubject, defer, zip, concat, of, debounceTime, delay, filter, finalize, first, map, scan, tap} = require('rxjs')
...
const metrics = {
    pending: createGauge({
        name: 'sepal_token_limiter_active_total',
        help: 'SEPAL token limiter active instances',
        labelNames: ['name']
    })
}
...
const TokenLimiter$ = ({name, rateWindowMs = DEFAULT_RATE_WINDOW_MS, maxRate, maxConcurrency, idleMs = DEFAULT_IDLE_MS}, onDispose) => {
    ...
    return (requestId = uuid()) => {
        ...
        const response$ = new ReplaySubject() // short-lived
        ...
        return defer(() => {
            metrics.pending.inc({name})
            requestId$.next(requestId)
            return response$.pipe(
                finalize(() => {
                    currentToken.recycleOrCancel()
                    metrics.pending.dec({name})
                })
            )
        })
    }
