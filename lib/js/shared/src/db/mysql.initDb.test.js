...
const validateMigrations = jest.fn()
...
const {initDb, migrateDb} = await import('./mysql.js')
...
beforeEach(() => {
    ...
    migrate.mockClear()
    validateMigrations.mockClear()
})

describe('created', () => {
    it('is false when the schema is already present', async () => {
        state.schemaCount = 1
        const {created} = await initDb('db', '/migrations')
...
describe('migrated and version', () => {
    it('migrates to the max version when the database is behind', async () => {
        ...
        state.maxVersion = 5
        const {migrated, version} = await initDb('db', '/migrations')
        expect(migrated).toBe(true)
        expect(version).toBe(5)
        expect(migrate).toHaveBeenCalled()
    })

    it('validates history without rerunning an up-to-date migration', async () => {
        ...
        expect(validateMigrations).toHaveBeenCalledWith(5)
    })
...
describe('migrateDb', () => {
    it('migrates without creating the database', async () => {
        state.databaseVersion = 2
        state.maxVersion = 5
        const result = await migrateDb('db', '/migrations')
        expect(result).toEqual({migrated: true, version: 5})
        expect(execute).not.toHaveBeenCalled()
    })

    it('does not migrate when the beforeMigrate hook fails', async () => {
        state.databaseVersion = 2
        state.maxVersion = 5
        const beforeMigrate = async () => {
            throw new Error('history rejected')
        }
        const migration = migrateDb('db', '/migrations', {beforeMigrate})
        await expect(migration).rejects.toThrow('history rejected')
        expect(migrate).not.toHaveBeenCalled()
    })
})
