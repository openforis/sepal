...
// The exact-first raster-mask path assumes north-up square pixels.
export const isAxisAlignedTransform = transform =>
    Array.isArray(transform) && transform.length === 6
        && transform[1] === 0 && transform[3] === 0
        && transform[0] !== 0 && Math.abs(transform[0]) === Math.abs(transform[4])
...
export const crsGridArgs = ({crs, scale, crsTransform}) => {
    const transform = parseCrsTransform(crsTransform)
    return transform
        ? {crs: crs || 'EPSG:3410', crsTransform: transform}
        : {crs: crs || 'EPSG:3410', scale}
}
...
// Single-pass stratified candidate label. The exact-first generator mosaics every stratum into one image and
// vectorizes once; reduceToVectors groups connected pixels by this label, so it must distinguish components
// across BOTH stratum and local lattice cell (i<<32+j alone collides between strata). Packs an int64
// [stratumIndex : i+2^(B-1) : j+2^(B-1)] with B = STRATIFIED_LABEL_BITS. Invariants: |i|,|j| < 2^(B-1) (holds
// for realistic projected global indices - EPSG:3410 spans < 2e7 m, so |i| < 2e7 << 2^25) and stratumIndex <
// 2^(63-2B). Uses the ALLOCATION INDEX (bounded, small) not the class value, so it can't overflow on large
// class ids. Returned as a decimal string (may exceed 2^53); the value is only a grouping key, stripped before
// final export, so client-side precision doesn't matter.
export const STRATIFIED_LABEL_BITS = 26
export const stratifiedCandidateLabel = (stratumIndex, i, j) => {
    const b = BigInt(STRATIFIED_LABEL_BITS)
    const offset = 1n << (b - 1n)
    return ((BigInt(stratumIndex) << (2n * b)) + ((BigInt(i) + offset) << b) + (BigInt(j) + offset)).toString()
}

// No-data never matches, including stratum 0.
