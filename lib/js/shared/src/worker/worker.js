...
const {filter, finalize, map, mergeMap, takeUntil, tap} = require('rxjs/operators')
const {serializeError} = require('serialize-error')
const _ = require('lodash')
const service = require('sepal/service')
const Transport = require('sepal/rxjs/transport')
...
const initWorker = ({workerId, jobName, port}) => {
    ...
    const setupJob = ({request$, response$}) => {
        ...
        const start = ({requestId, jobPath, initArgs, args}) => {
            ...
            const result$ = concat(...tasks$).pipe(
                ...
                takeUntil(stop$),
                finalize(() => {
                    log.debug(jobMsg(jobName, 'result$ finalized'))
                    stop()
                })
            )
        ...
        request$.pipe(
            takeUntil(stop$),
            finalize(() => {
                log.debug(workerMsg('request$ finalized'))
                stop()
            })
        ).subscribe({
            next: message => {
    ...
    const transport = Transport({port, transportId})
