const {Subject, of, catchError, finalize, map, mergeMap, takeUntil, tap, ReplaySubject, takeWhile} = require('rxjs')
...
const createScheduler = ({name, strategy, instances = 2, createConcurrency = 1, createDelayMs = 0}) => {
    ...
    request$.pipe(
        ...
        mergeMap(({username, jobName, jobPath, requestId, requestTag, initArgs, args, args$, cmd$, response$, cancel$, release$}) =>
            workerPool$(username, requestTag).pipe(
                tap(() => log.debug(jobMsg(requestTag, 'dequeued'))),
                mergeMap(submit$ =>
                    submit$({jobName, jobPath, requestId, requestTag, initArgs, args, args$, cmd$, cancel$}).pipe(
        ...
        next: ({response$, result: {next, error, complete, finalize, value}}) => {
            ...
            finalize && response$.next({finalize: true})
        },
    ...
    return {
        submit$: ({username, jobName, jobPath, requestId = uuid(), requestTag, initArgs, args, args$, cmd$}) => {
            ...
            setImmediate(() =>
                request$.next({username, jobName, jobPath, requestId, requestTag, initArgs, args, args$, cmd$, response$, cancel$, release$})
            )
