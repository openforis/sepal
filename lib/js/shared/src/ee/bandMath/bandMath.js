...
const bandMath = ({model}, {selection: selectedBands} = {selection: []}) => {
    ...
    const evaluateExpression = (calculation, eeImageById) => {
        const eeImageByName = _.mapKeys(eeImageById, (_eeImage, imageId) =>
    ...
    const getImage$ = () => {
        return forkJoin(images.reduce(
            ...
            map(eeImageById => {
                ...
                return Object.values(eeImageById).length
                    ? image.clip(ee.Geometry(ee.List(Object.values(eeImageById))
                        ...
                        .iterate(
                            (geometry, acc) => acc
                                ? ee.Algorithms.If(
                                    ...
                                    acc,
                                    ee.Geometry(acc).union(ee.Geometry(geometry), 1)
                                )
                                : geometry,
                            images[0].geometry()
                        )
