...
class UpdateTaskProgressHandler implements AfterCommitCommandHandler<Task, UpdateTaskProgress> {
    ...
    UpdateTaskProgressHandler(TaskGateway taskGateway, WorkerSessionManager sessionManager, CommandHandler<Task, CancelTask> cancelTaskHandler) {
    ...
    Task execute(UpdateTaskProgress command) {
        ...
        taskGateway.update(updatedTask)
        return updatedTask
    }
    ...
    void afterCommit(UpdateTaskProgress command, Task updatedTask) {
        ...
        def tasksInSession = taskGateway.pendingOrActiveTasksInSession(updatedTask.sessionId)
        if (!tasksInSession) {
