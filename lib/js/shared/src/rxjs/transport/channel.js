...
const channel = ({channelPort, transportId, conversationId, direction, in$ = new ReplaySubject(), out$ = new ReplaySubject()}) => {
    const stop$ = new ReplaySubject()
    const log = loggers[direction]

    const msg = (message, end) => [
        channelTag({transportId, conversationId, direction, end}),
        message
    ].join(' ')
