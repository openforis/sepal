...
// BASE_GRID_SLACK / ROOT_DIAMETER_EXPONENT and the diameter/spacing math are the canonical, unit-tested
// formulas; see systematicLatticeMath.js. Re-exported here for backwards compatibility.
import {BASE_GRID_SLACK, minLatticeExponent, ROOT_DIAMETER_EXPONENT, SQRT3, targetLatticeDiameter} from './systematicLatticeMath.js'
...
// cells, j: 32 cells). Moved to module scope (was nested in stratifiedSystematicSampleImage) so the
// unstratified index path reuses the exact same phase.
...
// Cell-coset "include" predicate for the nested-level computation. Moved to module scope, unchanged.
...
// Nested-level / half-level image band from the seed-shifted indices (iLevel, jLevel). Extracted verbatim
// from createHexSamplesImage so the raster and the unstratified index path share one level formula.
...
export function stratifiedSystematicSampleImage(args) {
    ...
    var originPhase = originPhaseOf(gridOrigin, seed)
    ...
    function createHexSamplesImage(args) {
        ...
        var sample = xMinDistance.lt(minDistanceFromCentroid)
            ...
            .rename('sample')
        var level = levelBand(iLevel, jLevel)
        // `i` and `j` are the global lattice indices for this stratum's lattice. They are threaded through
        // stratifiedSystematicSample()'s reduceToVectors() as first() properties so a candidate's
        // raster-snapped centroid can be replaced by the EXACT analytic point
        // (materializeStratifiedExactGeometry, using the same per-stratum dx/dy/offset/origin phase). `level`
        // alone can't reconstruct location; i + j + layout can. Threading the bands is behavior-neutral -
        // every existing consumer selects the bands/properties it needs (sample/level/stratum), so the extra
        // i/j are ignored until an exact-geometry post-process opts in. Replacing the exported geometry IS an
        // output change and must bump ALGORITHM_VERSION when routed into production.
        return sample
            .addBands(level)
            .addBands(i)
            .addBands(j)
...
// Per-stratum lattice layout for exact-point reconstruction, mirroring createHexSamplesImage: the client-side
// per-stratum diameter (same exponent math - targetLatticeDiameter, minLatticeExponent, densityOffset) then
// the EE dx/dy and origin-phase offsets. Each stratum's i/j live in ITS OWN lattice, so the layout must be
// rebuilt per stratum, keyed by the candidate's stratum.
function stratumExactLayout(stratum, sampleArrangement, densityOffset) {
    var exponent = Math.max(
        Math.floor(Math.log2(targetLatticeDiameter(stratum))) - (densityOffset || 0),
        minLatticeExponent({minDistance: sampleArrangement.minDistance, scale: sampleArrangement.scale})
    )
    var diameter = Math.pow(2, exponent)
    var proj = ee.Projection(sampleArrangement.crs || 'EPSG:3410', sampleArrangement.crsTransform || undefined)
    var nominalScale = proj.nominalScale()
    var distance = ee.Number(diameter).divide(nominalScale)
    var dx = distance.multiply(SQRT3)
    var dy = distance.multiply(1.5)
    var originPhase = originPhaseOf(sampleArrangement.gridOrigin || 'FIXED', ee.Number(sampleArrangement.seed || 0))
    var rootDistance = ee.Number(2).pow(ROOT_DIAMETER_EXPONENT).divide(nominalScale)
    var offsetX = originPhase.x.multiply(rootDistance.multiply(SQRT3)).mod(dx)
    var offsetY = originPhase.y.multiply(rootDistance.multiply(1.5)).mod(dy)
    return {proj: proj, dx: dx, dy: dy, offsetX: offsetX, offsetY: offsetY}
}

// Replace each candidate's raster-snapped centroid with the EXACT analytic lattice point computed from its
// stored i/j and its stratum's layout. Sign-safe row parity (((j%2)+2)%2) mirrors exactLatticePoint, so a
// negative odd j reconstructs the same point the raster's floor-based i identifies. Does NOT filterBounds: the
// caller decides whether to drop exact points that leave the AOI. Routed into the stratified systematic task
// export (systematicExport.js), at both the count and final stages, per stratum at the density offset that
// stratum's candidates were generated at. Emitting these exact points (instead of raster-snapped centroids) is
// the output change that bumped ALGORITHM_VERSION to v4.
export function materializeStratifiedExactGeometry(args) {
    ...
    return ee.FeatureCollection(allocation
        .map(function (stratum) {
            var layout = stratumExactLayout(stratum, sampleArrangement, densityOffset)
            var halfDx = layout.dx.divide(2)
            return candidates
                .filter(ee.Filter.eq('stratum', stratum.stratum))
                .map(function (feature) {
                    var i = ee.Number(feature.get('i'))
                    var j = ee.Number(feature.get('j'))
                    var parity = j.mod(2).add(2).mod(2)
                    var x = layout.offsetX.add(i.multiply(layout.dx)).add(parity.multiply(halfDx))
                    var y = layout.offsetY.add(j.multiply(layout.dy))
                    return feature.setGeometry(ee.Geometry.Point([x, y], layout.proj))
                })
        })
    ).flatten()
}

// Pre-count exact-membership filter: keep only candidates whose stratification class sampled AT THE EXACT
// POINT equals the candidate's own `stratum`. Preserves geometry and every existing property (id/row metadata
// untouched); reused for both base and repair candidate assets.
//
// Uses a per-feature reduceRegion, NOT sampleRegions, on purpose: the stratification band is itself named
// `stratum`, so sampleRegions would overwrite each candidate's own `stratum` property (breaking the
// comparison), whereas reduceRegion reads the value into a temporary without collision, keeps the input
// geometry/properties, and gives each feature its own timeout window (which scales in the batch candidate
// asset). A masked / off-image exact point reduces to null -> a negative sentinel (stratum classes are
// non-negative) -> never equals the candidate stratum -> dropped (mirrors the pure isExactMembershipMatch).
//
// Placement matters: the stratified task export (systematicExport.js) runs this before selectSystematicLevels
// at BOTH the base and repair count stages, so counts/levels reflect membership; the spike showed a
// post-selection filter breaks EXACT/OVER counts. The temporary `_exactMembership` flag is stripped from the
// returned collection. Routing this membership change is part of what bumped ALGORITHM_VERSION to v4.
export function filterToExactStratificationMembership(args) {
    var samples = args.samples
    var stratumImage = args.stratification.select('stratum')
    var scale = ee.Number(args.scale)
    var NO_DATA = -1
    return samples
        .map(function (sample) {
            var sampled = ee.Number(ee.List([
                stratumImage
                    .reduceRegion({reducer: ee.Reducer.first(), geometry: sample.geometry(), scale: scale})
                    .get('stratum'),
                NO_DATA
            ]).reduce(ee.Reducer.firstNonNull()))
            return sample.set('_exactMembership', sampled.eq(sample.getNumber('stratum')))
        })
        .filter(ee.Filter.eq('_exactMembership', 1))
        .map(function (sample) {
            return sample.set('_exactMembership', null)
        })
}
