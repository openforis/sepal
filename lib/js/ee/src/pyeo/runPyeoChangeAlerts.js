...
export const runPyeoChangeAlerts = params => {
  ...
  // This is a work in progress logic, so is turned OFF behind an if branch that is set as default to false   
  // until we can verify that there are no or low likelihood of actual changes being removed
  // *********
  ...
  if (hazeFilter.use) {
    ...
    var processAndFlagHaze = function(image) {
      ...
      var hasEnoughBaseline = baselineFromCount.gte(hazeFilter.fromAvailabilityThresholdPct);
      var isNotHazy = proportion.gt(hazeFilter.hazeLikelihoodThresholdPct);
  ...
  // assigns null to baselineIndex if not provided by SEPAL
  var baselineIndex = indexGate.use ? classifiedBaseline.select("gate_index") : null;
  ...
  var changeEvents = classifiedMonitoringCollection.map(function(image) {
    ...
    // initialise variables to update later
    var isChangeMask;
    var bandsToAdd = [];
    ...
    if (indexGate.use) {
      ...
      isChangeMask = transitionMask.and(deltaIndexThresholdedMask).rename("is_change");
        
      bandsToAdd.push(deltaIndexThresholdedMask, deltaIndex)
    }
      
    else {
      isChangeMask = transitionMask.rename("is_change")
    }
    ...
    return image.addBands(bandsToAdd);
  });

  // ==============================================================================
  // 3. LOGIC TO UNDERSTAND THE CONSISTENCY OF CHANGE
  // ==============================================================================
  ...
  // ==============================================================================
  // 4. COMPILING THE CHANGE REPORT LAYERS
  // ==============================================================================

  // LAYER 0: count the number of images within the collection
  var availableImageCount = ee.Image(classifiedMonitoringCollection.size())
    .clip(aoi)
  ...
  // if branching for index gating
  // initialise the layers that are required
  var deltaIndexChangeDetectionCount;
  var binaryDeltaIndexDecisionMap;
  var binaryCombinedDeltaDecisionMap;
  
  if (indexGate.use) {
    
    // LAYER 11: delta index only change detection count
    deltaIndexChangeDetectionCount = changeEvents
      .select("delta_index_thresholded_mask")
      .sum()
      .rename("delta_index_change_count");
  
    // LAYER 12: Binary delta index Alert Decision Map
    binaryDeltaIndexDecisionMap = deltaIndexChangeDetectionCount.gte(indexGate.minRequiredDeltaIndexDetectionsThreshold)
      .rename("binary_delta_index_decision_map");
      
    // LAYER 14: Combined delta index & delta class Decision Map
    binaryCombinedDeltaDecisionMap = binaryDeltaClassDecisionMap.and(binaryDeltaIndexDecisionMap)
      .rename("binary_combined_delta_decision_map");
  }
  // if no index gating supplied
  else {
    
    var baseMask = baselineClassification.mask();
    
    // LAYER 11 Placeholder
    deltaIndexChangeDetectionCount = ee.Image.constant(0)
      .rename("delta_index_change_count")
      .updateMask(baseMask);

    // LAYER 12 Placeholder
    binaryDeltaIndexDecisionMap = ee.Image.constant(0)
      .rename("binary_delta_index_decision_map")
      .updateMask(baseMask);

    // LAYER 14 Placeholder
    binaryCombinedDeltaDecisionMap = ee.Image.constant(0)
      .rename("binary_combined_delta_decision_map")
      .updateMask(baseMask);
  }
