...
test('removeRecipes soft-deletes by id list scoped to username', async () => {
    ...
    expect(sql).toMatch(/UPDATE processing_recipe\.recipe/i)
...
test('moveRecipes sets project_id for the id list scoped to username', async () => {
    ...
    const [sql, params] = query.mock.calls[0]
    expect(sql).toMatch(/UPDATE processing_recipe\.recipe SET project_id = \?/i)
    expect(sql).toMatch(/IN \(\?, \?\)/)
...
test('removeProject deletes the project then soft-deletes its recipes', async () => {
    ...
    expect(delSql).toMatch(/DELETE FROM processing_recipe\.project WHERE id = \? AND username = \?/i)
    expect(delParams).toEqual(['p1', 'bob'])
    expect(updSql).toMatch(/UPDATE processing_recipe\.recipe SET removed = TRUE WHERE project_id = \? AND username = \?/i)
...
test('listRecipesOfTypeBeforeVersion selects non-removed rows by type below version', async () => {
    ...
    const [sql, params] = query.mock.calls[0]
    expect(sql).toMatch(/SELECT.*FROM processing_recipe\.recipe WHERE type = \? AND type_version < \? AND NOT removed/is)
...
test('saveMigratedRecipe updates type_version and contents scoped to id and username', async () => {
    ...
    const [sql, params] = query.mock.calls[0]
    expect(sql).toMatch(/UPDATE processing_recipe\.recipe SET type_version = \?, contents = \? WHERE id = \? AND username = \?/i)
