...
// What calculateHarmonics composes for one dependent, in the order it adds them. The harmonics image selects
// by this list, so a name it does not produce fails here rather than in whatever declared it.
...
const calculateHarmonics = (collection, dependent) => {
    ...
    const amplitude = sin.hypot(cos).rename(`${dependent}_amp`)
    ...
    const harmonics = phase
        ...
        .addBands(t)
        .float()
        .select(HARMONIC_BAND_SUFFIXES.map(suffix => `${dependent}_${suffix}`))
    return {harmonics, fit}
}

export {addHarmonicBands, addHarmonics, calculateHarmonics}
