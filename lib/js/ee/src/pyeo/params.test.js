...
import {toClassifierBands, toUolParams} from './params.js'
...
test('classifier bands come from the band set specs, not the mosaic band menu', () => {
    const bands = toClassifierBands({
        bandSetSpecs: [{type: 'IMAGE_BANDS', included: ['blue', 'green', 'red', 'nir', 'swir1', 'ndvi']}]
    })
    assert.deepEqual(bands, ['blue', 'green', 'red', 'nir', 'swir1', 'ndvi'])
})

test('classifier bands merge every band set spec and drop duplicates', () => {
    const bands = toClassifierBands({
        bandSetSpecs: [
            {type: 'IMAGE_BANDS', included: ['red', 'nir']},
            {type: 'PAIR_WISE_EXPRESSION', included: ['red', 'swir1'], operation: 'RATIO'},
            {type: 'INDEXES', included: ['ndvi', 'nbr']}
        ]
    })
    assert.deepEqual(bands, ['red', 'nir', 'swir1', 'ndvi', 'nbr'])
})

test('classifier bands never include an index the data set cannot produce', () => {
    const bands = toClassifierBands({
        bandSetSpecs: [{type: 'IMAGE_BANDS', included: ['blue', 'red', 'nir', 'ndvi']}]
    })
    assert.ok(!bands.includes('ebbi'))
})

test('classifier bands fail loudly when there are no band set specs', () => {
    assert.throws(() => toClassifierBands({bandSetSpecs: []}), /no band set specs/)
    assert.throws(() => toClassifierBands({}), /no band set specs/)
})
