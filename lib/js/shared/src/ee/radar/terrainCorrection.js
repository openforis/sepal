...
var terrainCorrection = function(image, options) {
    ...
    // modified after Guido Lemoine's improved version for getting the azimuth/look_direction
    ...
    function _correct(image) {
        ...
        var alpha_sRad = ee.Terrain.slope(elevation).select('slope').multiply(Math.PI / 180).clip(geom)
        ...
        var phi_sRad = aspect
          .updateMask(aspect.lte(180)) // kee the values abvove 180 degree
          .unmask()                    // unmask the rest to 0
