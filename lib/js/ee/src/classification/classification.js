...
import {concat, defer, of, zip, map, switchMap, toArray} from 'rxjs'
...
const classify =
    ...
    {selection: selectedBands} = {selection: []}
        ...
        const normalizeImage = image => {
            if (classifierConfig.normalize === 'NO') {
                return image
            }
            const minMax = image.reduceRegion({
                ...
                tileScale: classifierConfig.tileScale || 1
            })
        ...
        const imageToClassify$ = defer(() => zip(
            ...
            map(imageToClassify => ['MINIMUM_DISTANCE', 'SVM'].includes(classifierConfig.type)
