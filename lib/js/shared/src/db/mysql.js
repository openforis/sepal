...
// One pooled connection, begun before the callback and committed after it: the callback's work is all or
// nothing, and the connection is released whatever happens.
export const createTransactionRunner = pool =>
    async callback => {
...
// What actually went wrong is more informative than the cleanup that also failed, so a rollback failure
// travels with the original rather than replacing it. A callback may throw anything at all - a frozen
// error, a primitive - and recording the rollback on it is best effort; the original stays authoritative.
const rollback = async (connection, error) => {
    try {
