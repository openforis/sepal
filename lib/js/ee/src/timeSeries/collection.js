...
import {collectionType, OPTICAL, planetSource, RADAR} from '#sepal/recipe/collectionType'

const CLASSIFICATION_BANDS = ['regression', 'probability_.*']

// A caller that already holds a geometry passes it in; otherwise the recipe's aoi is resolved first, so
// the collection is only ever constructed from a real geometry.
export const getCollection$ = ({recipe, geometry, startDate, endDate, bands}) =>
    defer(() => geometry ? of(geometry) : toGeometry$(recipe.model.aoi)).pipe(
        switchMap(resolvedGeometry =>
            toCollection$({recipe, geometry: resolvedGeometry, startDate, endDate, bands})
        )
    )

const toCollection$ = ({recipe, geometry, startDate: sd, endDate: ed, bands}) => {
    ...
    const radarImages = (classificationRecipe, trainingData) => {
        ...
        const processImage = image => {
            ...
            if (bands.includes('ratio_VV_VH')) {
                // VV and VH arrive in dB, so their ratio in dB is the difference, not the quotient.
                image = image
                    .addBands(
                        image.select('VV').subtract(image.select('VH')).rename('ratio_VV_VH')
                    )
            }
            return image
                .addBands(
    ...
    const planetImages = (classificationRecipe, trainingData) => {
        ...
        const collection = createPlanetCollection({
            ...
            endDate,
            geometry,
            sources: {...sources, source: planetSource(sources.dataSets)},
            histogramMatching,
            cloudThreshold,
            shadowThreshold,
            cloudBuffer
    ...
    return classificationRecipe$.pipe(
        switchMap(classificationRecipe =>
            (classificationRecipe
                ...
                : of(null)).pipe(
                map(trainingData => {
                    // The branch decision is shared with the code that declares this collection's
                    // dependencies, so the two cannot disagree about which sources are actually read.
                    switch (collectionType(dataSets)) {
                        case RADAR: return radarImages(classificationRecipe, trainingData)
                        case OPTICAL: return opticalImages(classificationRecipe, trainingData)
                        default: return planetImages(classificationRecipe, trainingData)
                    }
                })
            )),
        map(collection => {
