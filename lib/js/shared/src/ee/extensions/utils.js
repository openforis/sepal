...
module.exports = ee => {
    ...
    const $ = (operation, promiseCallback, id = uuid().substr(-4)) =>
        defer(() =>
            limiter$(
                from(new Promise(
                    (resolve, reject) => {
                        try {
                            log.trace(`Earth Engine <${operation}> starting [${id}]`)
                            const t0 = Date.now()
                            promiseCallback(result => {
                                const t1 = Date.now()
                                log.debug(`Earth Engine <${operation}> (${t1 - t0}ms) [${id}]`)
                                resolve(result)
                            }, reject)
                        } catch (error) {
                            log.error(`Earth Engine catched error while <${operation}> [${id}]`)
                            reject(error)
                        }
                    })
                ), id
