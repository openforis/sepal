...
def add_bands(image, ids, target_day_of_year, target_day_of_year_weight):
    ...
    return image.updateMask(ee.Image(0).where(image.select('QA60').gte(1024), 1).Not())
...
def quality_band(image, targetDay, target_day_of_year_weight):
    ...
    cloudQuality = image.metadata('CLOUDY_PIXEL_PERCENTAGE').divide(100).subtract(1).abs()
