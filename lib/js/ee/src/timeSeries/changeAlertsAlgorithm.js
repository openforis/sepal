...
// Given a segments image, the representation its dates are stored in and a monitoring collection, produces
// the alert bands. Written in the Earth Engine Code Editor's style.

// The bands this returns, in the order it builds them. Selecting by this list is what keeps the declaration
// and the image the same thing: a name no rename below produces fails here rather than reaching a consumer.
// `confidence` and `difference` are the two bands of one reduced image, renamed together.
...
export function analyzeChanges(args) {
