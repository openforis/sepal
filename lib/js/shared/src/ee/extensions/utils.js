const {concat, defer, of, switchMap} = require('rxjs')
...
module.exports = ee => {
    ...
    const createAssetFolders$ = (eeId, maxRetries) => {
        const folders = Asset({id: eeId}).getFolders()
        return concat(...folders.map(folder => {
            return defer(() => $({
                operation: `create asset directory(${folder})`,
    ...
    return {setMaxRetries, setUsername, $, getAssetRoots$, getAsset$, deleteAsset$, createAssetFolders$, getInfo$, getMap$, isNull, sepal}
}
