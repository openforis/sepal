...
const createProvisioning = exec => ({
    // Create the user's SSH home (+ keypair) and SEPAL data home; returns the SSH public key.
    provision: async (username, id) => {
        const {stdout} = await exec('sudo', [PROVISION, username, String(id)])
        return stdout.trim()
    },
