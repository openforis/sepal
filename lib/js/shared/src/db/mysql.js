...
// Migrates a database that already exists; initDatabase creates it first. `label` only names the
...
export const migrateDb = async (dbName, path, {schemaTable, beforeMigrate, label} = {}) => {
    if (!path) {
    ...
    try {
        ...
        await beforeMigrate?.(connection)
...
export const initDatabase = async (dbName, migrationsPath, options = {}) => {
