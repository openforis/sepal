...
const mosaic = (recipe, selectedBands, panSharpen) => {
    ...
    return {
        getImage$() {
            ...
            const image = addTasseledCap(mosaic, selectedBands)
                .select(selectedBands.length > 0 ? selectedBands : '.*')
                .clip(geometry)
            const ee = require('ee')
            log.warn(image.reduceRegion({reducer: ee.Reducer.first(), scale: 1000}).getInfo())
            return of(image)
        },
        getVisParams$() {
            if (selectedBands.length === 1 && selectedBands[0] === 'unixTimeDays') {
                const {seasonStart, seasonEnd, yearsBefore, yearsAfter} = dates
                ...
                return params
                    ? of(params)
                    : throwsError(new Error(`Unsupported band selection: ${selectedBands}`))
            }
        },
        getGeometry$() {
            return of(geometry)
