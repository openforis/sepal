...
const remap =
    ...
    {selection: selectedBands} = {selection: []}
        ...
        return {
            getImage$() {
                return zip(
                    ...images.map(image => imageFactory(image).getImage$())
                ).pipe(
                    map(eeImages => {
                        const eeImageById = {}
                        images.forEach(({id}, i) => eeImageById[id] = eeImages[i])
                        const image = ee.ImageCollection(entries
                            .reverse()
...
const toConstraintsMask = ({eeImageById, constraints = [], booleanOperator}) => {
    const constraintMasks = ee.ImageCollection(constraints
        .map(constraint => toConstraintMask({eeImageById, constraint}).int8())
    )
...
const toConstraintMask = ({eeImageById, constraint}) => {
    ...
    case 'class':
        return ee.ImageCollection(
            constraint.selectedClasses.map(value => image.eq(value))
