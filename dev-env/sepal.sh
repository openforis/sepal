...
SEPAL_MODULES=(user sepal-server api-gateway app-manager task gee gui user-storage terminal email sys-monitor)
...
group () {
    ...
    all)
        ...
        ;;
    dev)
        echo "user sepal-server ( -DskipSceneMetaDataUpdate ) api-gateway app-manager task gee gui user-storage terminal email sys-monitor"
        ;;
    *)
        return 1
        ;;
