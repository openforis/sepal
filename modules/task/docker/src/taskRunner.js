const {Subject, BehaviorSubject, concat, of} = require('rx')
const {distinctUntilChanged, filter, map, takeUntil, tap, finalize} = require('rx/operators')
...
const executeTask$ = ({id, name, params}) => {
    ...
    const submitTask = () => {
        ...
        const progress$ = concat(initialState$, task.submit$(id, params)).pipe(
    ...
    const taskFailed = (id, error) => {
        ...
        stateChanged$(id, 'FAILED', message).pipe(
            tap(() => log.fatal('TASK FAILED')),
            finalize(() => task$.error())
        ).subscribe({
    ...
    const taskCompleted = id => {
        ...
        stateChanged$(id, 'COMPLETED', message).pipe(
            tap(() => log.fatal('TASK COMPLETED')),
    ...
    return new BehaviorSubject('running').pipe(
        finalize(() => cancelTask())
    )
}
