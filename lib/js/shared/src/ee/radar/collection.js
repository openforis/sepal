...
const {leeFilter, gammaMap, refinedLee, leeSigma, snic} = require('./spatialSpeckleFilters')
...
const createCollection = ({
    ...
    const applyMultitemporalSpeckleFilter = spatialSpeckleFilter !== 'NONE' && multitemporalSpeckleFilter && multitemporalSpeckleFilter !== 'NONE'
    ...
    function resample() {
        return image => image
            .addBands(image.resample('bicubic'), null, true)
    }
    ...
    function applySpeckleFilter() {
        return image => {
            ...
            var speckleRatio = image.select(bandNames).divide(filtered.select(bandNames))
                .regexpRename('(.*)', '$1_speckle_ratio', false)
            return image
                .addBands(filtered, null, true)
                .addBands(speckleRatio, null, true)

            function filter(image) {
                if (!applyMultitemporalSpeckleFilter) {
    ...
    function applySpatialSpeckleFilter(image) {
        ...
        var retain = filter && retainStrongScatterers
            ? findStrongScatterers(image.select(bandNames))
            : ee.Image(0)
        var imagePixelsToFilter = image
            .select(bandNames)
            .updateMask(retain.not())
        var filtered = filter && filter(imagePixelsToFilter, {kernelSize, sigma, snicSize, snicCompactness, bandNames}) || image.select(bandNames)
    ...
    function findStrongScatterers(image) {
    ...
    function applyGeometricCorrection() {
        return image => {
            ...
            return correction && correction(image, null, bandNames) || image
        }
    }
    ...
    function maskPartialPixels() {
        // return image => image.updateMask(image.mask().gt(0.5))
        // return image => image.updateMask(image.mask().select(bandNames).reduce(ee.Reducer.min()).eq(1))
        return image => image.updateMask(image.mask().eq(1))
    }
}
