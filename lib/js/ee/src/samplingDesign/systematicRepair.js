...
const REASON_LEAD = {
    ...
    minDistanceLimit: 'Systematic sampling could not generate enough candidates. The grid is already at the minimum-distance limit, so no denser repair export can be attempted.',
    // One denser repair export was tried and still fell short (we do not attempt another).
    repairExhausted: 'Systematic sampling could not generate enough candidates after one denser repair export.'
}

const ACTIONS = 'Try reducing the requested sample size for those strata, reducing the minimum distance, using CLOSEST, or checking whether the AOI/stratification contains enough usable area for those classes.'
...
// Terminal underproduction message for EXACT/OVER: the reason (min-distance limit vs repair exhausted),
// each failing stratum (label + stratum, available/requested), and the actions the user can take.
export const underproductionMessage = ({details, reason}) =>
    `${REASON_LEAD[reason]} Underproducing strata: ${details.map(describeStratum).join('; ')}. ${ACTIONS}`
