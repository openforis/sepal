import {COLLECTION_AVAILABILITY} from './collectionAvailability.js'
...
const rules = [
    ...
    {
        name: 'datasetCollectionAvailability',
        // Closure surface is `/sources/dataSets` only — the topic the rule
        // asserts about. Listing `/dates/seasonEnd` in paths would activate
        // the rule whenever picking dates pulls seasonEnd writable, which
        // would cascade datasets → cloudMethods/corrections/companions far
        // beyond what this constraint requires. The predicate still reads
        // seasonEnd from the model; paths are the closure framework's
        // dependency tracker, separate from the predicate's data needs.
        // The rescope-on-validation retry handles the dynamic case: when
        // the runtime predicate fires, the named handle (datasets) is added
        // to picked and existing closure does the rest.
        description: 'Each selected dataset must have collection coverage within the analysis window [seasonStart, seasonEnd). Datasets whose collection starts on or after seasonEnd contribute no scenes and are invalid.',
        paths: ['/sources/dataSets'],
        subjectPaths: ['/sources/dataSets'],
        validate: model => {
            const seasonEnd = parseDate(model?.dates?.seasonEnd)
            if (!seasonEnd) return []
            const groups = model?.sources?.dataSets || {}
            const errors = []
            for (const codes of Object.values(groups)) {
                for (const code of codes || []) {
                    const floor = parseDate(COLLECTION_AVAILABILITY[code])
                    if (floor && seasonEnd <= floor) {
                        errors.push({
                            path: '/sources/dataSets',
                            message: `${code}: collection starts ${COLLECTION_AVAILABILITY[code]}, after analysis window end ${formatDate(seasonEnd)}`
                        })
                    }
                }
            }
