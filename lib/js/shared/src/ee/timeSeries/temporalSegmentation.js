...
function Segments(segmentsImage, dateFormat, maxSegments) {
    ...
    function interpolate(date, harmonics) {
        ...
        const startValue = startSegment.endFit(0)
        const endValue = endSegment.startFit(0)
        ...
        const t = ee.Image(toT(date)) // TODO: Allow t instead of date as arg?
        ...
        const phaseAndAmplitude = harmonicFit.phaseAndAmplitude(coefs, harmonics)
        ...
        const fit = segment.fit({harmonics: harmonics})
    ...
    function chartFeatures(features, callback) {
        features.evaluate(function (features, e) {
            ...
            try {
                ...
                const chart = ui.Chart({cols: cols, rows: rows}, 'LineChart', {
                    pointSize: 0,
                    lineWidth: 1.5,
                    series: {
