...
import {OPTICAL_STORED_PER_UNIT} from '#sepal/recipe/optical/encoding'
...
// Computed in physical units; the caller stores the result at the same rate.
const calculateIndex = (image, indexName) => indexes[indexName]
    ? indexes[indexName](image.divide(OPTICAL_STORED_PER_UNIT))
    : ee.Image([])

const supportedIndexes = () => Object.keys(indexes)

export {calculateIndex, supportedIndexes}
