...
export const changeBaseLayer = ({type, sepalMap, statePath, options}) => {
    actionBuilder('SET_BASE_LAYER', {type})
        .set([statePath, 'baseLayer'], type)
        .set([statePath, 'mapLayer'], options)
        .sideEffect(() => {
            const layer = createLayer({sepalMap, type, options})
            layer
...
const createLayer = ({sepalMap, type, options}) => {
    const {proc, dateRange, planetApiKey} = options
    switch (type) {
    case 'SEPAL':
        return null
    case 'GOOGLE_SATELLITE':
        return new GoogleSatelliteLayer({sepalMap, layerIndex: 0})
    case 'PLANET':
