...
import {createConnection, createDb, createPool, initDatabase} from '../../db/mysql.js'
...
export const createTestDb = async ({name, migrations}) => {
    ...
    try {
        ...
        await initDatabase(dbName, migrations)
        ...
        const pool = await ownPool(dbName, account)
...
const ownPool = (dbName, account) => createPool(dbName, {
    ...credentials(account),
    // One connection and no queue: an operation that failed to give its connection back, or one that
    // opened a second transaction of its own, fails outright rather than being served by another.
    connectionLimit: 1,
