// AUTHORIZATION: a caller may only report on a task that is both its user's and assigned to its own
// session. The check runs before every branch below, the CANCELING re-cancel included, so a refused
// caller leaves no state change and triggers no lifecycle effect.
//
...
const _execute = async ({taskId, state, statusDescription, username, sessionId}, deps) => {
    const {repo} = deps
    const task = await repo.getTask(taskId)
    assertAssignedTo(task, {username, sessionId})
...
// assertAssignedTo — the caller must be the executor this task was handed to. Another user's task
// and another of the caller's own sessions' tasks are refused alike.
const assertAssignedTo = (task, {username, sessionId}) => {
    if (storedUsername(task.username) !== storedUsername(username) || task.sessionId !== sessionId) {
        log.warn(`Refusing progress update for ${taskTag(task.id)} from ${sessionTag(sessionId)}`)
        throw new Unauthorized(`Task is not assigned to the calling session: ${task.id}`)
    }
}

// updateTaskProgress({taskId, state, statusDescription, username, sessionId}, deps) — authorize,
// persist, THEN side effect.
