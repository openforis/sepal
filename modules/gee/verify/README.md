...
| `declaredOutputBands.mjs` | Whether what a recipe says it provides is what it builds: `getBands$()` against the band names of the image `getImage$()` returns. Each fixture is asked three times, because they are different questions - asked for exactly the bands it declares, a producer must return those and no others; asked for a subset of them in an order it does not build in, it must return that subset in that order; and asked for its own arguments it builds whatever it builds, with anything beyond the declared list reported rather than judged. Covers the Change Alerts change product over an in-memory CCDC reference, a Radar Mosaic time scan with and without a harmonic-dependent selection, a Radar Mosaic point in time, and BAYTS Alerts over an in-memory BAYTS Historical reference, as both the alert product and a radar map mode. Read-only: recipes are held in memory and read through a RecipeScope of the run's own. Another recipe family is one more `FIXTURES` entry. |
