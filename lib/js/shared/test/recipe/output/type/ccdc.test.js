...
const derive = names => declaration()?.derive?.({observation: {bands: observed(names)}})
...
describe('the registered CCDC output declaration', () => {
    ...
    it('preserves observed physical dimensionality while retaining its declared policy', () => {
        expect(declaration()?.derive?.({observation: {
            bands: [{name: 'tStart', dataType: {arrayDimensions: 1}}]
        }})).toEqual({
            bands: [{
                name: 'tStart',
    ...
    it('does not modify the observation it was given', () => {
        const observation = {bands: observed(['tStart', 'ndvi_coefs'])}
