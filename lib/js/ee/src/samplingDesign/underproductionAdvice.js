...
// The single spacing recommendation that applies to this configuration, or null when none can help.
export const spacingAction = ({arrangementStrategy, minDistance, pixelSize, unstratified}) => {
    ...
    if (arrangementStrategy === 'RANDOM') {
        return distance > 0
            ? item('reduceRandomMinDistance',
                'Minimum distance is {minDistance} m. In Sample Arrangement, reduce or clear it to allow more sample locations.',
...
const statisticalMinimumAdvice = (strata, config) => {
    const {arrangementStrategy, minDistance} = config
    const systematic = arrangementStrategy !== 'RANDOM'
    const spacing = spacingAction(config)
    // Only claim the class itself is too rare once neither placement nor spacing can explain the shortage.
    const placementCouldExplain = systematic || (Number(minDistance) || 0) > 0
    const diagnosis = placementCouldExplain
