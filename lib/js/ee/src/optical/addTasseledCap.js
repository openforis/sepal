...
const addTasseledCap = (image, selectedBands) => {
    ...
    if (!selectedBands.length || tasseledCapBandsSelected) {
        ...
        const arrayImage = image.select(opticalBands).divide(OPTICAL_STORED_PER_UNIT).toArray().toArray(1)
        return image.addBandsReplace(
            ee.Image(coefs)
                .matrixMultiply(arrayImage)
                .arrayProject([0])
                .arrayFlatten([tasseledCapBands])
                .multiply(OPTICAL_STORED_PER_UNIT)
                .int16()
        )
    } else {
        return image
