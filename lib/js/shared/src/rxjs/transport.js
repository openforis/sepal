...
const Transport = ({transportId, messageOut, onChannel}) => {
    ...
    const removeChannelListener = ({key, conversationGroupId, conversationId, direction}) => {
        if (state.channelListeners[key]) {
            delete state.channelListeners[key]
    ...
    const createChannel = ({conversationGroupId, conversationId, direction, in$, out$}) => {
        ...
        return channel({channelPort, transportId, conversationGroupId, conversationId, direction, in$, out$})
    }

    const createChannelByCallback = ({conversationGroupId, conversationId, onChannel}) =>
        onChannel(
            createChannel({conversationGroupId, conversationId, direction: REVERSE})
        )

    const createChannelByCallbackMap = ({conversationGroupId, conversationId, handler}) =>
        handler(
            createChannel({conversationGroupId, conversationId, direction: REVERSE})
        )

    const createChannelByStreams = ({conversationGroupId, conversationId, in$, out$}) =>
        createChannel({conversationGroupId, conversationId, direction: REVERSE, in$, out$})

    const handleCreateChannel = ({transportId: currentTranportId, conversationGroupId, conversationId}, onChannel) => {
        if (_.isEqual(currentTranportId, transportId) && conversationGroupId && conversationId && onChannel) {
            if (_.isFunction(onChannel)) {
                createChannelByCallback({conversationGroupId, conversationId, onChannel})
            } else {
                const handler = onChannel[conversationGroupId]
                if (_.isFunction(handler)) {
                    createChannelByCallbackMap({conversationGroupId, conversationId, handler})
                } else if (_.isPlainObject(handler)) {
                    const {in$, out$} = handler
                    createChannelByStreams({conversationGroupId, conversationId, in$, out$})
                } else {
    ...
    return {
        createChannel: (conversationGroupId, {conversationId = uuid(), in$, out$} = {}) => {
            messageOut({createChannel: {transportId, conversationGroupId, conversationId}})
            return createChannel({conversationGroupId, conversationId, direction: FORWARD, in$, out$})
        },
        messageIn: message => {
