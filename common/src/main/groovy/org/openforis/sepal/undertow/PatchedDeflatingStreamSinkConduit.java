 ...
 */
public class PatchedDeflatingStreamSinkConduit implements StreamSinkConduit {
    ...
    public PatchedDeflatingStreamSinkConduit(final ConduitFactory<StreamSinkConduit> conduitFactory, final HttpServerExchange exchange) {
        this(conduitFactory, exchange, Deflater.DEFLATED);
    }

    protected PatchedDeflatingStreamSinkConduit(final ConduitFactory<StreamSinkConduit> conduitFactory, final HttpServerExchange exchange, int deflateLevel) {
    ...
    private StreamSinkConduit createNextChannel() {
        if (deflater.finished() && allAreSet(state, WRITTEN_TRAILER)) {
            ...
            if (isIdentityTransferEncoding()) // Content-Length is not allowed if Transfer-Encoding is other then identity
    ...
    /**
     * Determines if Transfer-Encoding is identity.
     */
    private boolean isIdentityTransferEncoding() {
        if (!exchange.getResponseHeaders().contains(Headers.TRANSFER_ENCODING))
            return true;
        HeaderValues transferEncodings = exchange.getResponseHeaders().get(Headers.TRANSFER_ENCODING);
