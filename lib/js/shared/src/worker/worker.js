    ...
    const runJob = ({request$, response$}) => {
        ...
        const start = ({requestId, requestTag, jobName, jobPath, initArgs, args}) => {
            ...
            const runWorker$ = (worker$, requestTag, initArgs, args, args$, cmd$) =>
                worker$({...args, requestId, requestTag, initArgs, args$, cmd$, state})
            ...
            const worker$ = tasks$.pipe(
                ...
                concatMap(({worker$, args}) =>
                    runWorker$(worker$, requestTag, initArgs, args, jobArgs$, jobCmd$)
                ),
                takeUntil(stop$),
                finalize(() => {
            ...
            const finalize$ = tasks$.pipe(
                ...
                concatMap(({finalize$, args}) =>
                    runWorker$(finalize$, requestTag, initArgs, args, jobArgs$, jobCmd$)
                ),
                finalize(() => {
                    workerLog(jobName, 'finalize$ finalized')
                    response$.next({requestId, finalize: true})
                    response$.complete()
                    stop()
                })
            )
