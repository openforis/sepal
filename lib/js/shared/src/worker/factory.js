...
const bootstrapWorker$ = ({workerId, logConfig}) => {
...
const setupWorker$ = ({workerId, worker}) => {
    ...
    const submit$ = ({jobName, jobPath, requestId, requestTag, initArgs, args, args$, cmd$}) => {
...
const initWorker$ = ({workerId, logConfig}) =>
    bootstrapWorker$({workerId, logConfig}).pipe(
