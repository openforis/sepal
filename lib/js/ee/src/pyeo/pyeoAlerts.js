...
const pyeoAlerts = (recipe, {selection = []} = {}) => {
    ...
    // The classification's own input imagery, loaded once: the bands the classifier references (so
    // monitoring can reproduce them) and its gate_index. RECIPE_REF mosaics compute the index band
    // directly; an ASSET can't synthesize bands, so we pull it and compute the index.
        ...
        loadRecipe$(classificationId).pipe(
            switchMap(classification => {
                ...
                return forkJoin({
                    ...
                    classifierBands: of(toClassifierBands(input))
                })
            })
        )
    ...
    const toChangeReport = ({geometry, classificationRecipe, trainingData, eeClassificationImage, eeBaselineGateIndex, monitoringCollection}) => {
        ...
        const image = selection.length ? changeReport.select(selection) : changeReport
    ...
    // An absent AOI means "no restriction" to the shared resolver, which for this recipe would be an
    // unbounded run rather than a mistake. Both operations that would act on a geometry refuse first,
    // before the classification is read or a collection is built.
    const geometry$ = () =>
        model.aoi?.type
            ? toGeometry$(model.aoi)
            : throwError(() => new ClientException('PyEO Alerts has no area of interest', {
                userMessage: {
