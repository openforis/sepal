...
// while ordered bands own their per-band export requirements. Evidence entries are opaque and retained
// by identity. Validation accumulates diagnostics and never returns a partial description. This module
// performs no resolution, policy inference or Earth Engine task conversion.
...
const bandDiagnostics = bands => {
    ...
    bands.forEach((band, index) => {
        ...
        const {name, pyramidingPolicy} = band
        if (isBlank(name)) {
...
export const imageOutputDescription = input => {
    ...
    return {
        description: {
            ...
            output: {
                kind: 'IMAGE',
                bands: bands.map(({name, pyramidingPolicy}) => ({name, pyramidingPolicy}))
            },
            evidence: [...evidence]
        },
        diagnostics
