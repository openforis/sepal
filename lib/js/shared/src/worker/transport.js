...
const transport = ({from = 'main', to = 'main', port}) => {
    ...
    const addChannelListener = ({key, listener, channelId, conversationId, direction, end}) => {
        if (state.channelListeners[key]) {
            ...
            log.debug(() => msg(`${channelTag(channelId, conversationId, direction, end)} added ${channelListenerTag(key)}, now ${_.keys(state.channelListeners).length}`))
        }
    }

    const removeChannelListener = ({key, channelId, conversationId, direction, end}) => {
        if (state.channelListeners[key]) {
            state.channelListeners = _.omit(state.channelListeners, key)
            log.debug(msg(`${channelTag(channelId, conversationId, direction, end)} removed ${channelListenerTag(key)}, now ${_.keys(state.channelListeners).length}`))
        } else {
            log.warn(msg(`${channelTag(channelId, conversationId, direction, end)} cannot remove non-existing ${channelListenerTag(key)}`))
        }
    }
    ...
    const createChannel = ({channelId, conversationId, direction, in$, out$}) => {
        ...
        const channelPort = {
            ...
            addMessageHandler: (end, messageHandler) => {
                ...
                addChannelListener({key, listener, channelId, conversationId, direction, end})
                return () => removeChannelListener({key, channelId, conversationId, direction, end})
            }
        }
