...
const mapCollection = ({targetDate, x, geometricCorrection, harmonicDependents}) =>
...
const applyGeometricCorrection = geometricCorrection =>
    image => {
        ...
        return correction && correction(image) || toDb(image)
    }

const toGamma0 = image => {
    const gamma0 = ee.Image().expression('i - 10 * log10(cos(angle * pi / 180))', {
        i: toDb(image).select(['VV', 'VH']),
        angle: image.select('angle'),
        pi: Math.PI
    })
    return ee.Image(
        image.addBands(gamma0, null, true)
            .copyProperties(image, image.propertyNames())
    )
}
