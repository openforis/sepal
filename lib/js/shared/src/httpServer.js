...
const requestLog = ({excludePaths = []}) =>
    async (ctx, next) => {
        if (excludePaths.includes(ctx.path)) {
            ...
            log.isInfo() && log.info(`${ctx.requestTag} ${ctx.method} ${ctx.url} - ${ctx.response.status}, ${ms}ms, ${ctx.request.length || 0}b → ${ctx.cancelled ? 'cancelled' : `${ctx.response.length || 0}b`}`)
        }
    }
