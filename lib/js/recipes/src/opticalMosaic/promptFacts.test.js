...
describe('MOSAIC.promptFacts — shape', () => {
    ...
    it('exposes the documented fields', () => {
        ...
        expect(Object.keys(facts).sort()).toEqual([
            'chooseWhen', 'description', 'dontChooseWhen', 'editGuidance', 'outputs', 'useCases'
        ])
    })
    ...
    it('returns editGuidance as a non-empty array of non-empty strings', () => {
        const facts = getRecipeSpec(MOSAIC).promptFacts()

        expect(Array.isArray(facts.editGuidance)).toBe(true)
        expect(facts.editGuidance.length).toBeGreaterThan(0)
        facts.editGuidance.forEach(guidance => {
