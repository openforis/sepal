...
const {mergeMap, map, filter, finalize, takeUntil, catchError, tap} = require('rx/operators')
...
const createJobScheduler = ({jobName, jobPath, logConfig, maxConcurrency, minIdleCount, maxIdleMilliseconds}) => {
        ...
        tap(({jobId, args}) => {
            ...
            log.isTrace()
                ? _.isEmpty(workerArgs)
                    ? log.trace(() => jobMsg(jobId, 'enqueued with no args'))
                    : log.trace(() => jobMsg(jobId, 'enqueued with args'), workerArgs)
                : log.debug(() => jobMsg(jobId, 'enqueued'))
        }),
        mergeMap(({jobId, initArgs, args, args$}) =>
            workerPool.getInstance$().pipe(
                tap(() => log.debug(jobMsg(jobId, 'dequeued'))),
                mergeMap(worker =>
                    worker.submit$({jobId, initArgs, args, args$}).pipe(
    ...
    return {
        submit$({initArgs, args, args$}) {
            const jobId = uuid()
            request$.next({jobId, initArgs, args, args$})
            return jobResponse$(
                response$.pipe(
                    filter(({jobId: currentJobId}) => currentJobId === jobId),
                    map(({result}) => result),
