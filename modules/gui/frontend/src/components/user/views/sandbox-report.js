...
var setSessions = function (sessions) {
  ...
  $.each(sessions, function (i, session) {
    ...
    var earliestTimeoutTime = row.find('[name=earliestTimeoutTime]')
    earliestTimeoutTime.val(numeral(session.earliestTimeoutHours).format('0.[00]'))
    earliestTimeoutTime.change(function () {
      EventBus.dispatch(Events.SECTION.USER.SET_SESSION_TIMEOUT, null, session.id, earliestTimeoutTime.val())
    })
