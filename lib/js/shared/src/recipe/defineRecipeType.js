...
//
// `segmentSource` and `historicalStatsSource` are the optional declarations a producer of CCDC segments, or
// of BAYTS historical statistics, makes about what it produces. A reader asks the producer rather than
// recognising producer types; each capability owns what its own declaration means.
//
...
export const defineRecipeType = definition => {
    const {type, directSources, imageOutput, segmentSource, historicalStatsSource} = validateRecipeType(definition)
