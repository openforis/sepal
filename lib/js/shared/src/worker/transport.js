...
const transport = ({id = uuid(), port}) => {
    ...
    const addChannelListener = (key, listener) => {
        if (state.channelListeners[key]) {
            ...
            log.debug(msg(`added channel listener [${key}], now ${_.keys(state.channelListeners).length}`))
        }
    }
    ...
    const removeChannelListener = key => {
        if (state.channelListeners[key]) {
            ...
            log.debug(msg(`removed channel listener [${key}], now ${_.keys(state.channelListeners).length}`))
        } else {
