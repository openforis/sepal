...
// enforced at the task boundary before the graph is built, so there is no CRS check here. The Stratification
// grid is {crs, scale}, and scale is metres.
function stratifiedSystematicExactCandidates(args) {
    ...
    var pixelSize = Number(stratificationGrid && stratificationGrid.scale)
    if (!(pixelSize > 0)) {
        throw new Error('stratifiedSystematicExactCandidates requires a positive stratificationGrid.scale in metres')
    }
    ...
    // changed, breaking the globally anchored nested lattice family.
