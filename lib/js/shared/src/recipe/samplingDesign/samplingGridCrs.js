...
// Sample placement defaults to EASE-Grid 2.0 Global. This must stay equal-area - moving it would silently change
// where every sample lands.
export const DEFAULT_SAMPLING_GRID_CRS = EASE_GRID_2_GLOBAL

// Stratification falls back to this when no grid can be derived from the source. EPSG:4326 so that using a recipe
// directly, and exporting it to an asset and using that, produce the same strata rather than silently differing.
export const DEFAULT_STRATIFICATION_CRS = 'EPSG:4326'
...
// field - scale - is carried through unchanged.
