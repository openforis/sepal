...
export function stratifiedSystematicSampleImage(args) {
    ...
    function createHexSamplesImage(args) {
        ...
        // FOLLOW-UP (exact locations for the STRATIFIED path): `i` and `j` are already the global lattice
        // indices here. Adding them to the returned image (`.addBands(i).addBands(j)`) would thread them
        // through stratifiedSystematicSample()'s reduceToVectors() as properties, so vectorized candidates
        // could have their raster-snapped centroid replaced by the exact analytic point
        // (systematicLatticeMath.exactLatticePoint, with the same dx/dy/offset/origin phase used here) - the
        // same technique the unstratified index path already uses. `level` alone can't reconstruct location;
        // i + j + layout can. This is an OUTPUT change and must bump ALGORITHM_VERSION when productionized.
        return sample
            .addBands(level)
            .updateMask(sample)
    }
