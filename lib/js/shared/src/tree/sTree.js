...
const assertNode = node =>
    assert(node, ({[NODE_KEY]: isNode}) => isNode === true, 'Tree: not a valid node', true)

const assertString = value =>
    assert(value, value => typeof value === 'string', `Tree: not a valid string: ${value}`, true)

const assertPath = value =>
    assert(value, value => Array.isArray(value), `Tree: not a valid path: ${value}`, true)

const assertNodeMapper = value =>
    assert(value, value => typeof value === 'function', 'Tree: not a valid node mapper', true)

const fromStringPath = (stringPath, ignoreEmptySegments = false) => {
    assertString(stringPath)
    if (stringPath === '') {
        return []
    }
    if (stringPath.startsWith('/') && !ignoreEmptySegments) {
        throw new Error('Absolute path not allowed')
    }
    const path = stringPath.split('/')
    if (path.some(segment => segment === '') && !ignoreEmptySegments) {
