...
function getEditFacts() {
    return {
        guidance: [
            ...
            'Source/correction dependency: selecting both Landsat and Sentinel-2 source groups requires cross-sensor calibration (`CALIBRATE`) and not surface reflectance (`SR`).',
            'Scene-selection dependency: selecting both Landsat and Sentinel-2 source groups requires all-scenes selection (`ALL`); remove `/scenes` unless scene selection is manually selected scenes (`SELECT`).',
