...
const landTrendr = (recipe, _args = {selection: []}) => {
    ...
    const toChangeMap = ltResult => {
        ...
        const mag = pick(magnitude, 'mag')
        // Per-pixel RMSE of the LandTrendr fit, in the same units as mag.
        // sig expresses the magnitude as a multiple of that fit error - a
        // change smaller than the noise the model couldn't explain (sig < 1)
        // is unlikely to be real. ee divide() yields 0 for rmse == 0.
        const rmse = ee.Image(ltResult).select('rmse')
        const sig = mag.abs().divide(rmse).rename('sig')
        return mag
