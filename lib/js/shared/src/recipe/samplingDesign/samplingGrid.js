...
// Grid pixel size in METRES. A transform is expressed in its CRS's units, which for EPSG:4326 are degrees, and
// isAxisAlignedTransform cannot tell the difference - it only inspects signs and squareness. A derived grid
// therefore carries the metre size explicitly (from projection().nominalScale()), and it wins; the transform is
// used only when no metre value was captured, and the scale last.
export const gridPixelSize = ({scale, crsTransform, pixelSizeMetres} = {}) => {
    const metres = Number(pixelSizeMetres)
    if (Number.isFinite(metres) && metres > 0) {
        return metres
    }
    // Falling back to the transform's own width is sound ONLY because a hand-authored transform is contractually
    // a projected-metre grid. A DERIVED transform can be in degrees, where abs(a) is ~9e-5 rather than 10 - that
    // is exactly the bug pixelSizeMetres exists to prevent, so this fallback is not generally safe.
