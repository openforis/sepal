...
const {Exception, SystemException, isException} = require('sepal/exception')
...
const initWorker = ({instanceId, jobName, port}) => {
    const args$ = new Subject()
    const stop$ = new Subject()

    const workerMsg = msg => [
        `Worker [${jobName}.${instanceId.substr(-4)}]`,
        msg
    ].join(' ')
    ...
    const setupJob = (response$, request$) => {
        const start = ({jobId, start: {jobPath, args}}) => {
            ...
            const jobMsg = (jobId, taskName, msg) =>
                workerMsg([
                    `running job [${jobName}.${jobId.substr(-4)}] task [${taskName}]`,
                    msg
                ].join(' '))
        ...
        request$.subscribe(
            ...
            // error => log.fatal(`Worker [${instanceId}] request stream failed:`, error),
...
parentPort.once('message', ({instanceId, jobName, logConfig, port}) => {
    exported.port = port
    configureLog(logConfig)
    initWorker({instanceId, jobName, port})
    ready()
})
