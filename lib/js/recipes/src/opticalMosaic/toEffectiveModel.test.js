import {getRecipeDefaults, getRecipeSpec, toEffectiveModel, validateRecipe} from '../index.js'
...
describe('MOSAIC.toEffectiveModel — projection rules', () => {
    ...
    it('rule 3b: preserves SR on mixed sources and does not add CALIBRATE', () => {
        const out = projected({
            sources: {cloudPercentageThreshold: 75, dataSets: {
                LANDSAT: ['LANDSAT_9'], SENTINEL_2: ['SENTINEL_2']
            }},
    ...
    it('rule 3b: preserves CALIBRATE on mixed sources and does not add SR', () => {
        const out = projected({
            sources: {cloudPercentageThreshold: 75, dataSets: {
                LANDSAT: ['LANDSAT_9'], SENTINEL_2: ['SENTINEL_2']
            }},
        ...
        expect(out.compositeOptions.corrections).toEqual(['CALIBRATE', 'BRDF'])
    })

    it('rule 3b: removes CALIBRATE when SR is also present', () => {
        const out = projected({
            sources: {cloudPercentageThreshold: 75, dataSets: {
                LANDSAT: ['LANDSAT_9'], SENTINEL_2: ['SENTINEL_2']
            }},
    ...
    it('rule 3b: leaves mixed-source corrections with neither SR nor CALIBRATE unchanged so validation fails downstream', () => {
        const stored = {
        ...
        const out = getRecipeSpec(MOSAIC).toEffectiveModel(stored)

        expect(out.compositeOptions.corrections).toEqual(['BRDF'])
        expect(validateRecipe(MOSAIC, out).some(e =>
            e.rule === 'mixedSourcesRequireSrOrCalibrate'
        )).toBe(true)
    })
