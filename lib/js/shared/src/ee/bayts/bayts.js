...
function bayts(args) {
    ...
    function postProcess(change) {
        ...
        var remappedFlag = flag
            .where(flag.eq(1).and(pChange.lt(lowConfidenceThreshold)), 1)
            .where(flag.eq(1).and(pChange.gte(lowConfidenceThreshold)), 2)
            .where(flag.eq(2).and(pChange.gte(lowConfidenceThreshold)), 3) // TODO: Do we need the pChange check for high conf?
        return change
                ...
                return prevFlag
                    ...
                    .where( // [2]
                        prevFlag.eq(LOW_CONF)
                            .and(pChange.lt(minChangeProbability))
                            .and(sameOrbitOrHighPnf),
                        NO_CHANGE
                    )
                    .where( // [3]
                        prevFlag.eq(LOW_CONF)
                            .and(pChange.gte(minChangeProbability).and(pChange.lt(highConfidenceThreshold)))
                            .and(sameOrbitOrHighPnf),
                        LOW_CONF
                    )
                    .where( // [4]
                        prevFlag.eq(LOW_CONF)
                            .and(pChange.gte(minChangeProbability).and(pChange.gte(highConfidenceThreshold)))
                            .and(sameOrbitOrHighPnf),
                        HIGH_CONF
                    )
                    .where( // [4b]
                    ...
                    .where( // [5]
                        prevFlag.eq(NO_CHANGE)
                            ...
                            .and(pChange.lt(highConfidenceThreshold)),
                        LOW_CONF
                    )
                    .where( // [6]
                        prevFlag.eq(NO_CHANGE)
                            .and(pnf.gte(minNonForestProbability))
                            .and(pChange.gte(highConfidenceThreshold)),
                        HIGH_CONF
                    )
                    .where( // [7]
                        prevFlag.eq(NO_CHANGE)
    ...
    function createMonitoringCollection() {
        ...
        function normalizeImage(image) {
            if (normalization === 'DISABLED') {
                console.log('no normalization', normalization)
                return image
            }
