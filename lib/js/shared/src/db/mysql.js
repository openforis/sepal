...
const ensureDatabaseExists = async database => {
    ...
    try {
        ...
        // The charset is explicit because the server default is latin1: without it a new schema
        // silently becomes latin1, and mysql2 connects as utf8mb4, so the first non-Latin-1
        // character is a hard error under STRICT_TRANS_TABLES. utf8mb4 (not utf8, which is 3-byte
        // utf8mb3) is the only charset that holds everything the client can send. Existing schemas
        // keep whatever charset they have — IF NOT EXISTS makes the clause a no-op for them.
