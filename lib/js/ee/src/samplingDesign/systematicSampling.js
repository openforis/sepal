...
// they are exported from the module below. stratifiedSystematicExactCandidates is kept textually identical to the
// retained Code Editor harness tmp/sudan-systematic/sudan-candidates-wkt-ij-bands-code-editor.js.
...
//
// One reduceToVectors over the compact label plus full i/j bands, then scalar enrichment in the same pass. The
// label is COMPACT (one 512-block per stratum + the i/j residues, so it stays below allocation.length * 512) and
// thus survives reduceToVectors' connected-component labeling without float64 rounding; it transports the stratum
// index and the nested-level residue. The full signed i/j travel as their own integer image bands, carried through
// the reducer as feature properties, and the temporary WGS84 centroid is discarded - a full Sudan export with
// native custom-WKT centroids exceeded Earth Engine's aggregation-result limit ("Computed value is too large"),
// while default centroids completed. The geoseeded origin, lattice spacing, marker coordinates and exact geometry
// all live in ONE projection: pixelCoordinates(crs[,transform]) with distances via nominalScale() - metres for a
// scale grid, pixel-index units for a transform grid. The origin is a seed-only global anchor (uniform over one
// period-aligned lattice period; independent of AOI/allocation/stratum/densityOffset); FIXED => origin (0,0).
//
...
function stratifiedSystematicExactCandidates(args) {
    ...
    // Vectorize the compact label plus the full i/j bands. `label` is the connected-component segment id; the
    // reducer carries each segment's full signed i/j as properties (first() is exact - every pixel of a segment
    // shares one marker's i/j). The temporary centroid is left in Earth Engine's default WGS84 representation and
    // then discarded in reconstruct(): a full Sudan export with native custom-WKT centroids
    // (geometryInNativeProjection:true) exceeded the aggregation-result limit ("Computed value is too large"),
    // while the default-centroid form completed. The reducer must be forEach(['i','j']); EE rejects
    // repeat(2).setOutputs(['i','j']) ("Need 1 output names for Reducer.repeat").
    var reduceArgs = {
