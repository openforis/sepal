...
describe('buildRecipeDependencyGraph', () => {
    ...
    it('includes a reached recipe whose type has no definition and reports it', () => {
        const result = graph(
            masking('root', {imageToMask: recipeRef('mosaic')}),
            [{id: 'mosaic', type: 'MOSAIC', model: {aoi: {type: 'RECIPE', id: 'aoi-recipe-1'}}}]
        )
        expect(recipeIds(result)).toEqual(['root', 'mosaic'])
        expect(result.diagnostics).toEqual([
            {code: 'UNSUPPORTED_RECIPE_TYPE', path: [], recipePath: ['root', 'mosaic']}
        ])
    })
