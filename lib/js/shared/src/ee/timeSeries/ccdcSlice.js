...
const ccdcSlice = (recipe, {selection: selectedBands, baseBands} = {selection: [], baseBands: []}) => {
    ...
    const createSlice = segmentsImage => {
        ...
        const rangeSlice = () => {
            ...
            const {breakAnalysisBand, breakMagnitudeDirection, minBreakConfidence, breakSelection} = model.options
            const breakpoint = dateRange.pickBreakpoint({
                breakAnalysisBand, breakMagnitudeDirection, minBreakConfidence, breakSelection
            })
