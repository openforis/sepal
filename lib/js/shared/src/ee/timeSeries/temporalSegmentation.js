...
function Segments(segmentsImage, dateFormat, maxSegments) {
    ...
    function interpolate(date, harmonics = 3) {
        ...
        const interceptCoefs = ee.Image(ee.Array([1, 0, 0, 0, 0, 0, 0, 0])).multiply(intercept)
        const slopeCoefs = ee.Image(ee.Array([0, 1, 0, 0, 0, 0, 0, 0])).multiply(slope)
        const weightedCoefs = startCoefs
            .multiply(tStartWeight).add(
                endCoefs.multiply(tEndWeight)
            )
        const harmonicCoefs = ee.Image(ee.Array([0, 0, 1, 1, 1, 1, 1, 1])).multiply(weightedCoefs)
...
function DateRange(segments, fromDate, toDate) {
    ...
    // const tStart = filteredSegments.toImage('tStart').max(segments.toT(fromDate))
    // const tEnd = filteredSegments.toImage('tEnd').min(segments.toT(toDate))

    return {
        mean
    }

    function mean(harmonics = 3) {
        const tStart = filteredSegments.toImage('tStart').max(filteredSegments.toT(fromDate))
        const tEnd = filteredSegments.toImage('tEnd').min(filteredSegments.toT(toDate))
        const coefs = filteredSegments.toImage('.*_coefs')
        const intercept = coefs.arraySlice(1, 0, 1).arrayProject([1]).regexpRename('(.*)_coefs', '$1_intercept', true)
        const slope = coefs.arraySlice(1, 1, 2).arrayProject([1]).regexpRename('(.*)_coefs', '$1_slope', true)
        const phases = sequence(1, harmonics).reduce(function (acc, harmonic) {
            const i = ee.Number(harmonic).multiply(2)
            const coef1 = coefs.arraySlice(1, i, i.add(1)).arrayProject([1])
            const coef2 = coefs.arraySlice(1, i.add(1), i.add(2)).arrayProject([1])
            return acc.addBands(
                coef1
                    .atan2(coef2)
                    .float()
        ...
        const amplitudes = sequence(1, harmonics).reduce(function (acc, harmonic) {
            const i = ee.Number(harmonic).multiply(2)
            const coef1 = coefs.arraySlice(1, i, i.add(1)).arrayProject([1])
            const coef2 = coefs.arraySlice(1, i.add(1), i.add(2)).arrayProject([1])
            return acc.addBands(
        ...
        const mean = harmonicSlice.meanArrayImage(coefs, tStart, tEnd, dateFormat, harmonics)
            .addBands(filteredSegments.toImage(['.*_rmse', 'numObs']))
            .addBands(intercept)
            .addBands(slope)
            .addBands(phases)
            .addBands(amplitudes)
        const duration = tEnd.subtract(tStart)
        const result = mean.multiply(duration).arrayReduce(ee.Reducer.sum(), [0])
            .divide(duration.arrayReduce(ee.Reducer.sum(), [0]))
            .arrayGet([0])
            .addBands(filteredSegments.first().toImage(['.*_magnitude', 'changeProb', 'tStart', 'tBreak']))
...
function sliceBandNames(coefs, harmonics) {
    const value = coefs.regexpRename('(.*)_coefs', '$1', true).bandNames()
    const rmse = coefs.regexpRename('(.*)_coefs', '$1_rmse', true).bandNames()
    const magnitude = coefs.regexpRename('(.*)_coefs', '$1_magnitude', true).bandNames()
    const intercept = coefs.regexpRename('(.*)_coefs', '$1_intercept', true).bandNames()
    const slope = coefs.regexpRename('(.*)_coefs', '$1_slope', true).bandNames()
    const phases = sequence(1, harmonics).map(function (i) {
        return coefs.regexpRename('(.*)_coefs', `$1_phase_${i}`, true).bandNames()
    })
    const amplitudes = sequence(1, harmonics).map(function (i) {
        return coefs.regexpRename('(.*)_coefs', `$1_amplitude_${i}`, true).bandNames()
    })
    return ee.List([
        'tStart', 'tEnd', 'tBreak', 'numObs', 'changeProb',
        value, rmse, magnitude, intercept, slope, phases, amplitudes
    ]).flatten()
}
