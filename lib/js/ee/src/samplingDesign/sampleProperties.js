...
// v3: unstratified systematic task export uses analytical index candidates and exact lattice-point geometry
//     instead of raster-snapped reduceToVectors centroids; EXACT thinning uses the stable lattice cell key
//     instead of a geometry-derived key. Stratified systematic task export remains on the raster path.
// v4: stratified systematic task export also moves to exact analytical lattice-point geometry (from i/j
//     threaded through the raster candidates) with pre-count exact-point stratification membership: level
//     counts/selection run on the membership-filtered exact candidates, and EXACT thinning is keyed by the
//     final exact geometry-derived id. Class membership and locations can differ from the raster-snapped v3
//     behaviour, so exports are not reproducible across the v3/v4 boundary.
export const ALGORITHM_VERSION = 'samplingDesign-v4'
