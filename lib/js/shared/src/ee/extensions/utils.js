...
module.exports = ee => {
    ...
    const listAssets$ = (parentEEId, maxRetries) => {
        ...
        const list$ = pageToken => $({
            ...
            catchError(error => {
                log.warning(`Error while trying to list assets for ${parentEEId}`, error)
                return of([])
            }),
    ...
    return {setMaxRetries, setUsername, $, getAssetRoots$, listAssets$, listOperations$, getAsset$, replaceAssetProperties$, deleteAsset$,
