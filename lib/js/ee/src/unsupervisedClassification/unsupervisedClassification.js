...
import {defer, of, zip, map} from 'rxjs'
    ...
    ({
        ...
        const normalizeImage = image => {
            const minMax = image.reduceRegion({
                ...
                tileScale: clustererConfig.tileScale || 1
            })
        ...
        const getTrainingData = imageToClassify => {
            return imageToClassify.sample({
                region: imageToClassify.geometry(),
        ...
        const imageToClassify$ = defer(() => zip(
            ...
            map(images =>
                clustererConfig.normalize === 'YES'
                    ? normalizeImage(ee.Image(images)).clip(images[0].geometry())
                    : ee.Image(images).clip(images[0].geometry())
            )
        )
...
const lvq = config =>
    ee.Clusterer.wekaLVQ({
        ...
        normalizeInput: config.normalize === 'YES'
    })
const xMeans = config =>
