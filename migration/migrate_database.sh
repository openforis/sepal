...
sleep 4
docker exec -it mysql mysql --host=localhost --user=root --password=$ROOT_PWD -e "CREATE USER 'sepal'@'%' IDENTIFIED BY '$SEPAL_PWD'"
docker exec -it mysql mysql --host=localhost --user=root --password=$ROOT_PWD -e "GRANT ALL PRIVILEGES ON sdms.* TO 'sepal'@'%'"
docker exec -it mysql mysql --host=localhost --user=root --password=$ROOT_PWD -e "FLUSH PRIVILEGES"
