const {Subject, of, filter, finalize, map, mergeMap, switchMap, tap, ReplaySubject, groupBy, merge, mergeScan, range, first, zip, timer, shareReplay} = require('rxjs')
...
const StaticPool = ({
    ...
    ).pipe(shareReplay(instanceCount))
        ...
        idleResponse$.pipe(
            ...
            next: instance => {
                ...
                if (ownInstance) {
                    ...
                    log.debug(requestTag, msg(instance, `using idle instance, now ${getActiveCount()} active`))

                    const l1 = _.size(Object.values(activePool))
                    const l2 = _.size(_.uniq(Object.values(activePool)))
                    l1 !== l2 && log.fatal('Duplicate values!', l1, l2)
    ...
    const idle = username => {
        ...
        log.debug(msg(instance, `idle (${username}), now ${getActiveCount()} active`))
        idlePool$.next(instance)
    }
