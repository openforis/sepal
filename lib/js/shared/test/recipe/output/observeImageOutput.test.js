...
const declarations = {
    ...
    [SAMPLED]: intrinsicImageOutput({
        derive: ({observation}) => ({
            bands: observedBands(observation).map(observed => declaredBand(observed, 'sample')),
            evidence: []
        })
    }),
    [CLASS_BASED]: intrinsicImageOutput({
        derive: ({observation}) => ({
            bands: observedBands(observation).map(observed =>
    ...
    [DUPLICATING]: intrinsicImageOutput({
        derive: ({observation}) => ({
            bands: observedBands(observation).map(() => band('same', 'mean')),
            evidence: []
        })
    }),
...
describe('observation boundary', () => {
    it('accepts the normalized observeBands$ contract without a legacy callback', () => {
        ...
        expect(requests).toEqual([{reference: recipeReference('root'), recipe: node('root', CLASS_BASED)}])
    })
