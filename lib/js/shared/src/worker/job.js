...
const workerThread = ({jobName, before, worker$}) =>
    _.flatten([...depWorker$(before), {jobName, worker$}])

const Job = ({jobName, jobPath, schedulerName, initArgs, before, services, worker$, args = () => []}) => {
    ...
    return ctx => isWorker(ctx)
        ? workerThread({jobName, before, worker$})
