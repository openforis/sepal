...
//
// Manual allocation is checked first because it has neither control: the user types a count into each stratum
// row, so it is sent there. `activeConfig` has already normalized manual to MANUAL and cleared any stale
// estimateSampleSize, and testing MANUAL before that flag keeps the precedence stated rather than incidental.
const reduceRequestedAction = ({arrangementStrategy, sampleSizeStrategy, estimateSampleSize, allocationStrategy}) => {
    ...
    if (allocationStrategy === 'MANUAL') {
        return canUseClosest
            ? item('reduceRequestedManualOrClosest',
                'In Sample Allocation, reduce the requested sample count for each affected stratum, or in Sample Arrangement choose Closest.')
            : item('reduceRequestedManual',
                'In Sample Allocation, reduce the requested sample count for each affected stratum.')
    }
