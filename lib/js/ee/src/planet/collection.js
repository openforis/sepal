...
import applyPercentileFilter from '#sepal/ee/optical/applyPercentileFilter'

import bufferClouds from '../optical/imageProcess/bufferClouds.js'
import {processBasemapCollection} from './basemap.js'
import {processDailyCollection} from './daily.js'
...
// The imagery a configured collection actually reads: every configured asset merged, then filtered by area
// and dates. A caller that must state what the collection carries stops here, rather than processing it.
export const contributingCollection = ({geometry, startDate, endDate, sources}) => {
...
const mergedCollection = ({geometry, startDate, endDate, sources, histogramMatching}) => {
    const collection = contributingCollection({geometry, startDate, endDate, sources})
