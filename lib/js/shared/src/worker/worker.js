...
const initLog = config =>
    require('sepal/log/configure').client(config)
    ...
    const setupJob = (response$, request$) => {
        const start = ({jobId, start: {jobPath, args}}) => {
            ...
            const result$ = concat(...tasks$).pipe(
                tap(({jobName, args}) => {
                    log.debug(msg(jobId, `running task [${jobName}]`))
                    log.trace(msg(jobId, `running task [${jobName}] with args:`), args)
                }),
...
parentPort.once('message', ({name, logConfig, port}) => {
    exported.port = port
    initLog(logConfig)
