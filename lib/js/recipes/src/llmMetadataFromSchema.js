...
function walk(node, root, prefix, constraints) {
    ...
    conditionalsOf(resolved).forEach(conditional => {
        const constraint = constraintFrom(conditional, resolved, prefix)
...
function constraintFrom({if: condition, then}, node, prefix) {
    ...
    const subjectPaths = required.map(field => `${prefix}/${field}`)
    const paths = distinct([...triggers.map(field => `${prefix}/${field}`), ...subjectPaths])
    const base = dotted(prefix)
    const triggerLabel = base ? `${base}.${trigger}` : trigger
    const valueLabel = valueLabelFor(node, trigger, value)
    return {
        name: `schema:${triggerLabel}${value === undefined ? '' : `=${value}`}`,
        description: value === undefined
            ? `${triggerLabel} set -> requires ${required.join(', ')}`
            : `${triggerLabel} includes ${displayValue(value, valueLabel)} -> requires ${required.join(', ')}`,
        paths,
        subjectPaths
    }
}

function valueLabelFor(node, trigger, value) {
    if (value === undefined) return null
    const property = node.properties?.[trigger]
    const labels = property?.['x-enumLabels'] || property?.items?.['x-enumLabels']
