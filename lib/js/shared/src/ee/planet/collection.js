...
const createCollection = (
    ...
    return mergedCollection({geometry, startDate, endDate, sources})
        .map(function (image) {
            const ndvi = image
...
const hasImagery = ({geometry, startDate, endDate, sources}) =>
    mergedCollection({geometry, startDate, endDate, sources})
        .filter(filter({geometry, startDate, endDate}))
        .isEmpty()
        .not()

const mergedCollection = ({geometry, startDate, endDate, sources}) => {
    ...
    return sources.source === 'DAILY'
        ? processDailyCollection({
            ...
            startDate,
            endDate,
            histogramMatching: sources.histogramMatching
        })
