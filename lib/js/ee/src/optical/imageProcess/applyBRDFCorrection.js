...
import * as gp from './geometryParser.js'

const expr = ee.Image.expr
const tileOrbits = () => ee.FeatureCollection('users/wiell/SepalResources/sentinel2TileOrbits')

const applyBRDFCorrection = (dataSetSpec, brdfMultiplier = 4) =>
    image => {
        const dataSetName = dataSetSpec.dataSetName
        return dataSetName === 'SENTINEL_2'
            ? ee.Algorithms.If(
                getTileOrbit(image),
                correct(image, () => getSentinel2ViewAngles(image), brdfMultiplier),
                image
            )
            : correct(image, () => getLandsatViewAngles(image, dataSetSpec.maxZenith), brdfMultiplier)
    }
...
const correct = (image, getViewAngles, brdfMultiplier) => {
    ...
    const adjustedBands = adjustBands({image, kvol, kvol0, kgeo, kgeo0, brdfMultiplier})
...
const getSunZenOut = geometry => {
    ...
    const centerLat = ee.Number(geometry.centroid(30).coordinates().get(1)).float()
    return expr(
        `31.0076
            - 0.1272 * centerLat
...
const adjustBands = ({image, kvol, kvol0, kgeo, kgeo0, brdfMultiplier}) => {
    const COEFFICIENTS = {
    ...
    return ee.Image(
        _.map(COEFFICIENTS, (coefficients, bandName) =>
            image.withBand(bandName, band =>
                applyCFactor(band, {kvol, kvol0, kgeo, kgeo0, coefficients, brdfMultiplier})
            )
        )
    )
}

const applyCFactor = (band, {kvol, kvol0, kgeo, kgeo0, coefficients, brdfMultiplier}) => {
    const brdf = getBrdf(kvol, kgeo, coefficients, brdfMultiplier)
    const brdf0 = getBrdf(kvol0, kgeo0, coefficients, brdfMultiplier)
    return expr('band * brdf0 / brdf', {band, brdf, brdf0})
}

const getBrdf = (kvol, kgeo, coefficients, brdfMultiplier) =>
    expr('fiso + multiplier * fvol * kvol + fgeo * kgeo', {
        ...coefficients,
        kvol,
        kgeo,
        multiplier: toFloat(brdfMultiplier)
    }) /* We multiply by factor to strenghten the correction*/ // We multiply by factor to strenghten the correction
...
export default applyBRDFCorrection
