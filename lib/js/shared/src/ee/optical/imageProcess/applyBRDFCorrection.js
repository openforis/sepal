...
const applyBRDFCorrection = () =>
    image => {
        const dataSetSpec = ee.Dictionary(image.get('dataSetSpec'))
        const dataSetName = dataSetSpec.getString('dataSetName')
        return ee.Image(ee.Algorithms.If(
            dataSetName.match('SENTINEL_2').length(),
            ee.Algorithms.If(
                getTileOrbit(image),
                correct(image, () => getSentinel2ViewAngles(image)),
                image
            ),
            correct(image, () => getLandsatViewAngles(image, dataSetSpec.getNumber('maxZenith')))
        ))
    }
