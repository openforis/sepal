...
const baytsAlerts = (
    ...
    if (visualizationType && visualizationType !== 'alerts') {
        ...
        return {
            getImage$() {
                return toAlerts$(recipe, reference, args)
                
            },
            getBands$() {
                return toAlerts$(recipe, reference, args).pipe(
...
const toAlerts$ = (recipe, reference, args) => {
    ...
    return zip(historicalStats$, initialAlerts$).pipe(
        map(([historicalStats, initialAlerts]) => {
            const alerts = bayts({
                ...toDates(recipe),
            ...
            return mask(recipe, alerts, args)
        })
    )
}

const mask = (recipe, alerts, {previouslyConfirmed, minConfidence}) => {
